...
@Component("orderContractExpensesAction")
@Scope("prototype")
public class OrderContractExpensesAction extends BaseController{
	...
	public String orderContractExpenses() throws Exception {
		...
		if(bussiType==1){
			return "editOrderContractExpenses";
		}
		if(bussiType==2){
			return "trialerOrderContractExpenses";
		}
		return "editOrderContractExpenses";
	}
	/**
	...
	public void saveOrderContractDecore() throws Exception {
		...
		Integer orderContractExpensesBuyCarType = ParamUtil.getIntParam(request2, "orderContractExpensesBuyCarType", 1);
		...
		try{
			...
			Integer loanFinStatus = customer.getLoanFinStatus();
			if(loanFinStatus==null){
				customer.setLoanFinStatus(1);
			}else{
				if(loanFinStatus==1){
					customer.setLoanType(orderContractExpenses.getLoanPaymentType());
				}
			}
			orderContractExpenses.setBuyCarType(orderContractExpensesBuyCarType);
			if(orderContractExpenses.getDbid()==null||orderContractExpenses.getDbid()<0){
				...
				customer.setLoanType(orderContractExpenses.getBuyCarType());
				orderContractManageImpl.save(orderContract);
				customerOperatorLogManageImpl.saveCustomerOperatorLog(customer.getDbid(), "创建合同费用明细","");
			}else{
				//销售顾问编辑数据、或物流部编辑数据
				...
				orderContractExpenses2.setBuyCarType(orderContractExpensesBuyCarType);
				...
				orderContractExpenses2.setFinProductItemName(orderContractExpenses.getFinProductItemName());
				orderContractExpenses2.setFinProductName(orderContractExpenses.getFinProductName());
				Double totalGrofit = orderContractExpenses.getTotalGrofit();
				orderContractExpenses2.setTotalGrofitPrice(totalGrofit);
				orderContractExpensesManageImpl.save(orderContractExpenses2);
				//删除重复数据
				orderContractExpensesManageImpl.deleteDuplicateDataByCustomerId(customerId);
				customerOperatorLogManageImpl.saveCustomerOperatorLog(customer.getDbid(), "编辑合同费用明细","");
				//保存优惠项目
				savePreferenceItemProduct(request2, orderContractExpenses);
				//保存收费项目
				saveChargeItemProduct(request2, orderContractExpenses);
		...
		if(editType==1){
			renderMsg("/orderContractDecore/orderContractDecore?customerId="+customerId+"&editType="+editType, "保存数据成功,正在跳转到附加装饰通知单页面！");
			return ;
		}
		//3跳转到物流编辑
		if(editType==3){
			renderMsg("/outboundOrder/orderContractDecore?customerId="+customerId+"&editType="+editType+"&modifyType="+modifyType, "保存数据成功,正在跳转到附加装饰通知单页面！");
			return ;
		}
		//默认跳转到填写附件通知单
		renderMsg("/orderContractDecore/orderContractDecore?customerId="+customerId+"&editType="+editType, "保存数据成功,正在跳转到附加装饰通知单页面！");
