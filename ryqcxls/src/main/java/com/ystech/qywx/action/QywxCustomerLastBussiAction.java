 ...
 * @date 2014-2-25
 */
@Component("qywxCustomerLastBussiAction")
@Scope("prototype")
public class QywxCustomerLastBussiAction extends BaseController{
	...
	public void save() throws Exception {
		...
		Integer buyCarType = ParamUtil.getIntParam(request, "buyCarType", -1);
	...
	public void turnOutFlowerCustomerToCustomer() throws Exception {
		HttpServletRequest request = getRequest();
		Integer dbid = ParamUtil.getIntParam(request, "custDbid", -1);
		try{
			if(dbid>0){
				//删除成交记录信息
				...
				if(null!=recommendCustomer){
					...
					recommendCustomerManageImpl.save(recommendCustomer);
					saveAgentOperatorLog("流失客户转为登记客户", "推荐客户流失转为登记客户", customer.getDbid(), user);
				}
				//推荐客户修改状态为客户流失
				//保存客户操作日志
				customerOperatorLogManageImpl.saveCustomerOperatorLog(customer.getDbid(), "流失客户转为登记客户", "",user);
			}else {
		...
		renderMsg("/qywxCustomer/queryOutFlow", "流失客户已经转换为登记客户，请到登记客户页面查看！");
		return ;
	}
	/**
	 * 功能描述：保存操作日志
