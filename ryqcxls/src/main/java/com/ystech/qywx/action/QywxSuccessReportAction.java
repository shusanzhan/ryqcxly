...
@Component("qywxSuccessReportAction")
@Scope("prototype")
public class QywxSuccessReportAction extends BaseController{
	...
	public String customerSuccess() throws Exception {
		...
		try{
			...
			String todayCarSerirySql="SELECT cs.dbid as serid,cs.`name` as serName,COUNT(cpid.carSeriyId) as countNum " +
					...
					"WHERE" +
					"	cust.dbid=cpid.customerId AND cpid.carSeriyId=cs.dbid AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+" AND cpid.modifyTime>='"+DateUtil.format(start)+"' AND cpid.modifyTime<'"+DateUtil.format(end)+"'"+
					" GROUP BY cs.dbid ";
			...
			String brandSql="SELECT br.dbid as serid,br.`name` as serName,COUNT(cpid.brandId) as countNum " +
					...
					"WHERE" +
					"	cust.dbid=cpid.customerId AND cpid.brandId=br.dbid AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+" AND cpid.modifyTime>='"+DateUtil.format(start)+"' AND cpid.modifyTime<'"+DateUtil.format(end)+"'"+
					" GROUP BY br.dbid ";
			...
			String netTotalSql="SELECT " +
					...
					"WHERE" +
					" dep.dbid=cust.successDepartmentId AND cpid.customerId=cust.dbid AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+
					" AND cpid.modifyTime>='"+DateUtil.format(start)+"' AND cpid.modifyTime<'"+DateUtil.format(end)+"' and cust.customerType=2";
			...
			String countUserByDepId="SELECT " +
					"dep.dbid as dbid,dep.`name` AS depName,COUNT(dep.dbid) totalNum " +
					"FROM	sys_department dep,cust_customer cust,cust_customerpidbookingrecord cpid " +
					"WHERE" +
			...
			if(null!=netDepart){
				String netDepartmentSql="SELECT * FROM department WHERE parentId="+netDepart.getDbid();
			...
			String netCountUserByDepId="SELECT " +
					...
					"WHERE" +
					" dep.dbid=cust.successDepartmentId AND dep.parentId=paramDepId AND cpid.customerId=cust.dbid  AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+
					" AND cpid.modifyTime>='"+DateUtil.format(start)+"' AND cpid.modifyTime<'"+DateUtil.format(end)+"' and cust.customerType=2 GROUP BY dep.dbid";
	...
	public String customerSuccess7Day() throws Exception {
		...
		try{
			...
			/*****今天客户情况统计***/
			String newsCountSql="SELECT COUNT(*) as total FROM cust_customer cust,cust_customerpidbookingrecord cpid where cust.dbid=cpid.customerId AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+" And cpid.modifyTime>='"+DateUtil.format(start)+"' AND cpid.modifyTime<'"+DateUtil.format(end)+"'";
			//新增加客户
			...
			String sql="SELECT DATE_FORMAT(cpid.modifyTime,'%m-%d') AS createFolderTime,COUNT(DATE_FORMAT(cpid.modifyTime,'%y-%m-%d')) AS totalNum  "
					...
					"  WHERE TO_DAYS(NOW()) - TO_DAYS(cpid.modifyTime) <7 AND cust.dbid=cpid.customerId AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+
			...
			String totalWeekSql="SELECT " +
					"COUNT(*) as total " +
					"FROM " +
					"customer cust,customerpidbookingrecord cpid " +
					"WHERE " +
					" TO_DAYS(NOW()) - TO_DAYS(cpid.modifyTime) <7 AND cust.dbid=cpid.customerId AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED;
			...
			String comeTypeSql="SELECT " +
					"COUNT(*) as total " +
					"FROM " +
					"customer cust,customershoppingrecord csr,customerpidbookingrecord cpid " +
					"WHERE " +
					"csr.customerId=cust.dbid AND cpid.customerId=cust.dbid AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+" AND csr.comeType=paramType AND TO_DAYS(NOW()) - TO_DAYS(cpid.modifyTime) <7 ";
			//来店
			String comeShopSql = "SELECT " +
					...
					"customer cust,customershoppingrecord csr,customerpidbookingrecord cpid " +
					"WHERE " +
					"csr.customerId=cust.dbid AND cpid.customerId=cust.dbid AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+" AND (csr.comeType=1 or csr.comeType is null) AND TO_DAYS(NOW()) - TO_DAYS(cpid.modifyTime) <7 ";
			...
			String carserySql="SELECT " +
					...
					"WHERE " +
					"cust.dbid=cpid.customerId AND cs.dbid=cpid.carSeriyId AND TO_DAYS(NOW()) - TO_DAYS(cpid.modifyTime) <7 AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+
			...
			if(null!=carSerCountsTop5&&carSerCountsTop5.size()>0){
				...
				for (CarSerCount carSerCount : carSerCountsTop5) {
					String carserDateSql="SELECT " +
							...
							"WHERE " +
							"cust.dbid=cpid.customerId AND cpid.carSeriyId="+carSerCount.getSerid()+" AND TO_DAYS(NOW()) - TO_DAYS(cpid.modifyTime) <7 AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+
							" GROUP BY DATE_FORMAT(cpid.modifyTime,'%Y-%m-%d')";
					if(i==0){
						buffCarseryValues.append("{");
			...
			String countUserByDepId="SELECT " +
					"dep.dbid as dbid,dep.`name` AS depName,COUNT(dep.dbid) totalNum " +
					"FROM " +
					"cust_customer cust,sys_department dep,cust_customerpidbookingrecord cpid  " +
					"WHERE " +
					"dep.dbid=cust.successDepartmentId AND dep.parentId=paramDepId AND TO_DAYS(NOW()) - TO_DAYS(cpid.modifyTime) <7 AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+" AND cpid.customerId=cust.dbid " +
					"GROUP BY dep.dbid";
			request.setAttribute("countUserByDepId", countUserByDepId);
				...
				if (result.size()>0) {
					...
					for (Department department : result) {
						String departSql="SELECT " +
								...
								"cust_customer cust,cust_customerpidbookingrecord cpid " +
								"WHERE " +
								"departmentId="+department.getDbid()+" AND TO_DAYS(NOW()) - TO_DAYS(cpid.modifyTime) <7 AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+" AND cpid.customerId=cust.dbid " +
								"GROUP BY DATE_FORMAT(cpid.modifyTime,'%Y-%m-%d')";
						if(i==0){
							buffDepartmentValues.append("{");
		...
		String netTotalSql="SELECT " +
				...
				"WHERE" +
				" dep.dbid=cust.successDepartmentId AND cpid.customerId=cust.dbid AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+
				" AND  TO_DAYS(NOW()) - TO_DAYS(cpid.modifyTime) <7 and cust.customerType=2";
		Object netTotal = statisticalSalerManageImpl.queryCount(netTotalSql);
		request.setAttribute("netTotal", netTotal);
		...
		String netCountUserByDepId="SELECT " +
				...
				"WHERE" +
				" dep.dbid=cust.successDepartmentId AND dep.parentId=paramDepId AND cpid.customerId=cust.dbid  AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+
				" AND  TO_DAYS(NOW()) - TO_DAYS(cpid.modifyTime) <7 and cust.customerType=2 GROUP BY dep.dbid";
		request.setAttribute("netCountUserByDepId", netCountUserByDepId);
	...
	public String customerSuccessWeek() throws Exception {
		...
		try{
			...
			/*****今天客户情况统计***/
			String newsCountSql="SELECT COUNT(*) as total FROM cust_customer cust,cust_customerpidbookingrecord cpid "
					+ "where cust.dbid=cpid.customerId AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+" ANd WEEKOFYEAR(CURDATE())=WEEKOFYEAR(cpid.modifyTime) AND YEAR(CURDATE())=YEAR(cpid.modifyTime)";
			//新增加客户
			Object newsCount = statisticalSalerManageImpl.queryCount(newsCountSql);
			request.setAttribute("newsCount", newsCount);
			///////////////////////////////////////////////////////////////////////////一周集客趋势图//////////////////////////////////////////////////////////////////
			...
			String sql="SELECT " +
					...
					"cust_customer cust,cust_customerpidbookingrecord cpid  " +
					"  WHERE " +
					"(WEEKOFYEAR(CURDATE())-WEEKOFYEAR(cpid.modifyTime))<7 AND  YEAR(CURDATE())=YEAR(cpid.modifyTime) AND cust.dbid=cpid.customerId And cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+
			...
			String comeTypeSql="SELECT " +
					...
					"WHERE " +
					"csr.customerId=cust.dbid AND cpid.customerId=cust.dbid AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+" AND csr.comeType=paramType And (WEEKOFYEAR(CURDATE())-WEEKOFYEAR(cpid.modifyTime))<7 AND  YEAR(CURDATE())=YEAR(cpid.modifyTime) ";
			//来店
			String comeShopSql = "SELECT " +
					"COUNT(*) as total " +
					"FROM " +
					"cust_customer cust,cust_customershoppingrecord csr,cust_customerpidbookingrecord cpid " +
					"WHERE " +
					"csr.customerId=cust.dbid AND cpid.customerId=cust.dbid AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+" AND (csr.comeType=1 or csr.comeType is null) AND (WEEKOFYEAR(CURDATE())-WEEKOFYEAR(cpid.modifyTime))<7 AND  YEAR(CURDATE())=YEAR(cpid.modifyTime) ";
			Object comeShopNum = statisticalSalerManageImpl.queryCount(comeShopSql);
			request.setAttribute("comeShopNum", comeShopNum);
			//来电
			String comePhoneSql = comeTypeSql.replace("paramType", "2");
			...
			String carserySql="SELECT " +
					...
					"set_carseriy cs,cust_customer cust,cust_customerpidbookingrecord cpid " +
					"WHERE " +
					"cust.dbid=cpid.customerId AND cs.dbid=cpid.carSeriyId AND (WEEKOFYEAR(CURDATE())-WEEKOFYEAR(cpid.modifyTime))<7 AND  YEAR(CURDATE())=YEAR(cpid.modifyTime) AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+
					" GROUP BY cs.dbid ORDER BY countNum DESC ";
			List<CarSerCount> carSerCountsTop5 = statisticalManageImpl.querStatic(carserySql);
			request.setAttribute("carSerCountsTop5", carSerCountsTop5);
			StringBuffer buffCarseryValues=new StringBuffer();
			if(null!=carSerCountsTop5&&carSerCountsTop5.size()>0){
				...
				for (CarSerCount carSerCount : carSerCountsTop5) {
					String carserDateSql="SELECT " +
							...
							"cust_customer cust,cust_customerpidbookingrecord cpid " +
							"WHERE " +
							"cust.dbid=cpid.customerId AND cpid.carSeriyId="+carSerCount.getSerid()+" AND (WEEKOFYEAR(CURDATE())-WEEKOFYEAR(cpid.modifyTime))<7 AND  YEAR(CURDATE())=YEAR(cpid.modifyTime) AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+
							" GROUP BY WEEKOFYEAR(cpid.modifyTime)";
					if(i==0){
						buffCarseryValues.append("{");
						buffCarseryValues.append("name:'"+carSerCount.getSerName()+"',");
			...
			String countUserByDepId="SELECT " +
					...
					"cust_customer cust,sys_department dep,cust_customerpidbookingrecord cpid  " +
					"WHERE " +
					"dep.dbid=cust.successDepartmentId AND dep.parentId=paramDepId AND (WEEKOFYEAR(CURDATE())-WEEKOFYEAR(cpid.modifyTime))<7 AND  YEAR(CURDATE())=YEAR(cpid.modifyTime) AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+" AND cpid.customerId=cust.dbid " +
					"GROUP BY dep.dbid";
			request.setAttribute("countUserByDepId", countUserByDepId);
				...
				if (result.size()>0) {
					...
					for (Department department : result) {
						String departSql="SELECT " +
								...
								"cust_customer cust,cust_customerpidbookingrecord cpid " +
								"WHERE " +
								"departmentId="+department.getDbid()+" AND (WEEKOFYEAR(CURDATE())-WEEKOFYEAR(cpid.modifyTime))<7 AND  YEAR(CURDATE())=YEAR(cpid.modifyTime) AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+" AND cpid.customerId=cust.dbid " +
								"GROUP BY WEEKOFYEAR(cpid.modifyTime)";
						if(i==0){
							buffDepartmentValues.append("{");
							buffDepartmentValues.append("name:'"+department.getName().replace("销售部","")+"',");
		...
		String netTotalSql="SELECT " +
				...
				"WHERE" +
				" dep.dbid=cust.successDepartmentId AND cpid.customerId=cust.dbid AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+
				" AND   (WEEKOFYEAR(CURDATE())-WEEKOFYEAR(cpid.modifyTime))<7 AND  YEAR(CURDATE())=YEAR(cpid.modifyTime) and cust.customerType=2";
		Object netTotal = statisticalSalerManageImpl.queryCount(netTotalSql);
		request.setAttribute("netTotal", netTotal);
		...
		String netCountUserByDepId="SELECT " +
				...
				"WHERE" +
				" dep.dbid=cust.successDepartmentId AND dep.parentId=paramDepId AND cpid.customerId=cust.dbid  AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+
				" AND  (WEEKOFYEAR(CURDATE())-WEEKOFYEAR(cpid.modifyTime))<7 AND  YEAR(CURDATE())=YEAR(cpid.modifyTime) and cust.customerType=2 GROUP BY dep.dbid";
		request.setAttribute("netCountUserByDepId", netCountUserByDepId);
	...
	public String customerSuccessMonth() throws Exception {
		...
		try{
			...
			/*****今天客户情况统计***/
			String newsCountSql="SELECT COUNT(*) as total FROM "
					+ "cust_customer cust,cust_customerpidbookingrecord cpid"
					+ " where cust.dbid=cpid.customerId AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+" And cpid.modifyTime>='"+DateUtil.format(new Date())+"' AND cpid.modifyTime<'"+DateUtil.format(new Date())+"'";
			...
			String comeTypeSql="SELECT " +
					...
					"cust_customer cust,cust_customershoppingrecord csr,cust_customerpidbookingrecord cpid " +
					"WHERE " +
					"csr.customerId=cust.dbid AND cpid.customerId=cust.dbid AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+" AND csr.comeType=paramType And  DATE_FORMAT(cpid.modifyTime,'%Y-%m')='"+start+"' ";
			//来店
			String comeShopSql = "SELECT " +
					"COUNT(*) as total " +
					"FROM " +
					"cust_customer cust,cust_customershoppingrecord csr,cust_customerpidbookingrecord cpid " +
					"WHERE " +
					"csr.customerId=cust.dbid AND cpid.customerId=cust.dbid AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+" AND (csr.comeType=1 or csr.comeType is null) AND  DATE_FORMAT(cpid.modifyTime,'%Y-%m')='"+start+"' ";
			Object comeShopNum = statisticalSalerManageImpl.queryCount(comeShopSql);
			request.setAttribute("comeShopNum", comeShopNum);
			//来电
			String comePhoneSql = comeTypeSql.replace("paramType", "2");
			...
			String carserySql="SELECT " +
					...
					"set_carseriy cs,cust_customer cust,cust_customerpidbookingrecord cpid " +
					"WHERE " +
					"cust.dbid=cpid.customerId AND cs.dbid=cpid.carSeriyId AND DATE_FORMAT(cpid.modifyTime,'%Y-%m')='"+start+"' AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+
					" GROUP BY cs.dbid ORDER BY countNum DESC ";
			List<CarSerCount> carSerCountsTop5 = statisticalManageImpl.querStatic(carserySql);
			request.setAttribute("carSerCountsTop5", carSerCountsTop5);
			StringBuffer buffCarseryValues=new StringBuffer();
			if(null!=carSerCountsTop5&&carSerCountsTop5.size()>0){
				...
				for (CarSerCount carSerCount : carSerCountsTop5) {
					String carserDateSql="SELECT " +
							...
							"cust_customer cust,cust_customerpidbookingrecord cpid " +
							"WHERE " +
							"cust.dbid=cpid.customerId AND cpid.carSeriyId="+carSerCount.getSerid()+" AND  DATE_FORMAT(cpid.modifyTime,'%Y-%m')='"+start+"' AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+
							" GROUP BY DATE_FORMAT(cpid.modifyTime,'%d')";
					if(i==0){
						buffCarseryValues.append("{");
						buffCarseryValues.append("name:'"+carSerCount.getSerName()+"',");
			...
			String countUserByDepId="SELECT " +
					...
					"cust_customer cust,sys_department dep,cust_customerpidbookingrecord cpid  " +
					"WHERE " +
					"dep.dbid=cust.successDepartmentId AND dep.parentId=paramDepId AND  DATE_FORMAT(cpid.modifyTime,'%Y-%m')='"+start+"' AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+" AND cpid.customerId=cust.dbid " +
					"GROUP BY dep.dbid";
			request.setAttribute("countUserByDepId", countUserByDepId);
				...
				if (result.size()>0) {
					...
					for (Department department : result) {
						String departSql="SELECT " +
								...
								"cust_customer cust,cust_customerpidbookingrecord cpid " +
								"WHERE " +
								"departmentId="+department.getDbid()+" AND  DATE_FORMAT(cpid.modifyTime,'%Y-%m')='"+start+"' AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+" AND cpid.customerId=cust.dbid " +
								"GROUP BY DATE_FORMAT(cpid.modifyTime,'%d')";
						if(i==0){
							buffDepartmentValues.append("{");
							buffDepartmentValues.append("name:'"+department.getName().replace("销售部","")+"',");
		...
		String netTotalSql="SELECT " +
				...
				"WHERE" +
				" dep.dbid=cust.successDepartmentId AND cpid.customerId=cust.dbid AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+
				" AND DATE_FORMAT(cpid.modifyTime,'%Y-%m')='"+start+"' and cust.customerType=2";
		Object netTotal = statisticalSalerManageImpl.queryCount(netTotalSql);
		request.setAttribute("netTotal", netTotal);
		...
		String netCountUserByDepId="SELECT " +
				...
				"WHERE" +
				" dep.dbid=cust.successDepartmentId AND dep.parentId=paramDepId AND cpid.customerId=cust.dbid  AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+
				" AND  DATE_FORMAT(cpid.modifyTime,'%Y-%m')='"+start+"' and cust.customerType=2 GROUP BY dep.dbid";
		request.setAttribute("netCountUserByDepId", netCountUserByDepId);
	...
	public String customerSuccessYear() throws Exception {
		...
		try{
			...
			/*****今天客户情况统计***/
			String newsCountSql="SELECT COUNT(*) as total FROM cust_customer cust,cust_customerpidbookingrecord cpid where cust.dbid=cpid.customerId AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+" And MONTH(cpid.modifyTime)=MONTH(CURDATE()) and YEAR(cpid.modifyTime)='"+start+"'";
			//新增加客户
			Object newsCount = statisticalSalerManageImpl.queryCount(newsCountSql);
			request.setAttribute("newsCount", newsCount);
			///////////////////////////////////////////////////////////////////////////一周集客趋势图//////////////////////////////////////////////////////////////////
			...
			String comeTypeSql="SELECT " +
					...
					"cust_customer cust,cust_customershoppingrecord csr,cust_customerpidbookingrecord cpid " +
					"WHERE " +
					"csr.customerId=cust.dbid AND cpid.customerId=cust.dbid AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+" AND csr.comeType=paramType And  YEAR(cpid.modifyTime)='"+start+"'";
			//来店
			String comeShopSql = "SELECT " +
					"COUNT(*) as total " +
					"FROM " +
					"cust_customer cust,cust_customershoppingrecord csr,cust_customerpidbookingrecord cpid " +
					"WHERE " +
					"csr.customerId=cust.dbid AND cpid.customerId=cust.dbid AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+" AND (csr.comeType=1 or csr.comeType is null) AND  YEAR(cpid.modifyTime)='"+start+"'";
			Object comeShopNum = statisticalSalerManageImpl.queryCount(comeShopSql);
			request.setAttribute("comeShopNum", comeShopNum);
			//来电
			String comePhoneSql = comeTypeSql.replace("paramType", "2");
			...
			String carserySql="SELECT " +
					...
					"set_carseriy cs,cust_customer cust,cust_customerpidbookingrecord cpid " +
					"WHERE " +
					"cust.dbid=cpid.customerId AND cs.dbid=cpid.carSeriyId AND YEAR(cpid.modifyTime)='"+start+"' AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+
					" GROUP BY cs.dbid ORDER BY countNum DESC ";
			List<CarSerCount> carSerCountsTop5 = statisticalManageImpl.querStatic(carserySql);
			request.setAttribute("carSerCountsTop5", carSerCountsTop5);
			StringBuffer buffCarseryValues=new StringBuffer();
			if(null!=carSerCountsTop5&&carSerCountsTop5.size()>0){
				...
				for (CarSerCount carSerCount : carSerCountsTop5) {
					String carserDateSql="SELECT " +
							...
							"cust_customer cust,cust_customerpidbookingrecord cpid " +
							"WHERE " +
							"cust.dbid=cpid.customerId AND cpid.carSeriyId="+carSerCount.getSerid()+" AND  YEAR(cpid.modifyTime)='"+start+"' AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+
							" GROUP BY MONTH(cpid.modifyTime)";
					if(i==0){
						buffCarseryValues.append("{");
						buffCarseryValues.append("name:'"+carSerCount.getSerName()+"',");
			...
			String countUserByDepId="SELECT " +
					...
					"cust_customer cust,sys_department dep,cust_customerpidbookingrecord cpid  " +
					"WHERE " +
					"dep.dbid=cust.successDepartmentId AND dep.parentId=paramDepId AND  YEAR(cpid.modifyTime)='"+start+"' AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+" AND cpid.customerId=cust.dbid " +
					"GROUP BY dep.dbid";
			request.setAttribute("countUserByDepId", countUserByDepId);
				...
				if (result.size()>0) {
					...
					for (Department department : result) {
						String departSql="SELECT " +
								...
								"cust_customer cust,cust_customerpidbookingrecord cpid " +
								"WHERE " +
								"departmentId="+department.getDbid()+" AND  YEAR(cpid.modifyTime)='"+start+"' AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+" AND cpid.customerId=cust.dbid " +
								"GROUP BY MONTH(cpid.modifyTime)";
						if(i==0){
							buffDepartmentValues.append("{");
							buffDepartmentValues.append("name:'"+department.getName().replace("销售部","")+"',");
		...
		String netTotalSql="SELECT " +
				...
				"WHERE" +
				" dep.dbid=cust.successDepartmentId AND cpid.customerId=cust.dbid AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+
				" AND YEAR(cpid.modifyTime)='"+start+"' and cust.customerType=2";
		Object netTotal = statisticalSalerManageImpl.queryCount(netTotalSql);
		request.setAttribute("netTotal", netTotal);
		...
		String netCountUserByDepId="SELECT " +
				...
				"WHERE" +
				" dep.dbid=cust.successDepartmentId AND dep.parentId=paramDepId AND cpid.customerId=cust.dbid  AND cpid.pidStatus="+CustomerPidBookingRecord.FINISHED+
				" AND  YEAR(cpid.modifyTime)='"+start+"' and cust.customerType=2 GROUP BY dep.dbid";
		request.setAttribute("netCountUserByDepId", netCountUserByDepId);
