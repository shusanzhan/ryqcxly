...
@Component("statCustomerRecordAction")
@Scope("prototype")
public class StatCustomerRecordAction extends BaseController{
	...
	private void receptionCarSerData(HttpServletRequest request,
		...
		dataBuf.append("[{");
		if(type==1){
			dataBuf.append("name:'来店关注车型',");
		}
		if(type==2){
			dataBuf.append("name:'来电关注车型',");
		}
		dataBuf.append("data:[");
		int i=0;
		...
		dataBuf.append("]");
		dataBuf.append("}]");
		request.setAttribute("receptionCarSerTitle", titleBuf.toString());
