...
<div class="listOperate">
	<div class="operate">
  	...
  	<div class="seracrhOperate">
  		<form name="searchPageForm" id="searchPageForm"  action="${ctx}/factoryOrder/queryWlbStock" method="post" >
  				...
  				<td>
  					<select class="text small" id="carModelId" name="carModelId"  onchange="$('#searchPageForm')[0].submit()">
						...
						<c:forEach var="carModel" items="${carModels }">
							<option value="${carModel.dbid }" ${param.carModelId==carModel.dbid?'selected="selected"':'' } >${carModel.name }[${carModel.navPrice }]</option>
						</c:forEach>
					</select>
  				</td>
			...
			<td>
				<c:if test="${factoryOrder.enterprise.dbid==enterprise.dbid }" var="status">
					<c:if test="${factoryOrder.reserveStatus==2 }">
						...
						<a href="${ctx }/factoryOrder/factoryOrderDetail?dbid=${factoryOrder.dbid}&vinCode=${factoryOrder.vinCode }&type=6" class="" style="color: blue;"><span style="color: blue;">查看订单</span></a>
					</c:if>
					<c:if test="${factoryOrder.reserveStatus==3 }">
						...
						<a href="javascript:void(-1)" onclick="$.utile.openDialog('${ctx }/specialMotorPool/view?vinCode=${factoryOrder.vinCode}','占车仓库',920,450)"  class="aedit" style="color: green;">占车信息</a>
					</c:if>
					<c:if test="${factoryOrder.reserveStatus==1 }">
						...
						<a href="javascript:void(-1)" class="aedit" onclick="$.utile.openDialog('${ctx }/specialMotorPool/quickAdd?factoryOrderId=${factoryOrder.dbid}','快速占车',920,500)" >快速占车</a>
					</c:if>
				</c:if>
				<c:if test="${status==false }">
					<span style="color: red;">分公司车辆</span>
				</c:if>
			</td>
		</tr>
		</c:forEach>
