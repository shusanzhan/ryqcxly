...
<input type="hidden" value="${installDecoration.dbid}" id="dbid" name="installDecoration.dbid">
							...
							<div id='te' class="widget-content nopadding" style="height: 300px;overflow: scroll;padding-bottom: 20px;">
								<table id="shopNumber" class="table table-bordered" >
									...
									<tbody style="overflow: scroll;">
										<c:set value="0" var="numIndex"></c:set>
										<c:forEach var="installItem" begin="0" items="${installitems }" varStatus="in">
											<c:set value="${in.index }" var="numIndex"></c:set>
											<tr id="tr${numIndex+1}">
												<td style="text-align: center;">
													${numIndex+1}
												</td>
												<td >
													<input type="text" name="productName" id="productName${numIndex+1}" value="${installItem.itemName }" onFocus="autoProductByName('productName${numIndex+1}');create(this)"  style="width: 100%;">
													<input type="hidden" name="productDbid" id="productDbid${numIndex+1}" value="${installItem.product.dbid }" onblur="create(this)" style="width: 100%;">
												</td>
												<td id="sn${numIndex+1}" style="text-align: center;">
													<span>${installItem.serNo }</span>
												</td>
												<td id="brand${numIndex+1}" style="text-align: center;">
													${installItem.product.brand.name }
												</td>
												<td id="cbj${numIndex+1}" style="text-align: center;">
													${installItem.product.costPrice }
												</td>
												<td style="text-align: center;">
													<input type="text" readonly="readonly"  name="price" id="price${numIndex+1}" value="${installItem.price }" style="width: 92%;" onfocus="count()">
												</td>
												<td>
													<input type="text" name="num" id="num${numIndex+1}" value="${installItem.quality}" style="width: 92%;" onfocus="count()" onchange="count()" checkType="integer" canEmpty="Y" tip="请输入商品数量，必须为数字">
												</td>
										...
										<c:forEach var="sec" begin="${numIndex+1 }" step="1" end="7">
											<tr id="tr${sec+1 }">
												<td style="text-align: center;">
													${sec+1 }
												</td>
												<td >
													<input type="text" name="productName" id="productName${sec+1 }" onFocus="autoProductByName('productName${sec+1 }');create(this)"  style="width: 100%;">
													<input type="hidden" name="productDbid" id="productDbid${sec+1}" onblur="create(this)" style="width: 100%;">
												</td>
												<td id="sn${sec+1 }" style="text-align: center;">
													<span></span>
												</td>
												<td id="brand${sec+1 }" style="text-align: center;"></td>
												<td id="cbj${sec+1 }" style="text-align: center;"></td>
												<td style="text-align: center;">
													<input type="text" readonly="readonly"  name="price" id="price${sec+1 }" style="width: 92%;" onfocus="count()">
												</td>
												<td>
													<input type="text" name="num" id="num${sec+1 }" style="width: 92%;" onfocus="count()" onchange="count()" checkType="integer" canEmpty="Y" tip="请输入商品数量，必须为数字">
												</td>
												<td style="text-align: center;padding-top: 8px;">
													<a class="aedit" href="javascript:void(-1)" onclick="deleteTr(this)" >
														<i class="icon-remove icon-black" > </i>
...
<div class="container-fluid" style="margin-top: 0;">
	<table style="width: 100%"  >
			...
			<c:if test="${installDecoration.installType==1 }" var="status">
			<tr id="systemInstallTypeTr">
				<td width="80">统装类型</td>
				<td width="120">
			...
			<tr>
				<td width="12%">加装名称</td>
				<td width="88%">
					<input style="width: 240px;" type="text" id="installName" name="installDecoration.installName" value="${installDecoration.installName }" >
				</td>
			</tr>
			<tr>
				<td width="80">加装日期</td>
				<td width="120">
					<input type="text" style="width: 240px;" readonly="readonly" id="installDate" name="installDecoration.installDate" value="${installDecoration.installDate }" title="还款日期" onfocus="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd'});"	checkType="string,1,50">
				</td>
			</tr>
			<tr>
				<td width="80">备注</td>
				<td colspan="">
					<textarea style="width:440px;height: 60px;"  id="note" name="installDecoration.note" >${installDecoration.note }</textarea>
				</td>
			</tr>
			<tr>
				<td colspan="2" align="left" style="padding-right: 12px;padding-top: 12px;">
					<a id="submit" class="btn btn-primary" href="javascript:void(-1)" onclick="if(vali()){$.utile.submitAjaxForm('frmId','${ctx}/installDecoration/update')}">
					<i	class="icon-ok-sign icon-white"></i>保存</a>
					<c:if test="${!empty(installDecoration) }">
						<a class="btn btn-danger" onclick="$.utile.operatorDataByDbid('${ctx }/installDecoration/canncelInstall?dbid=${installDecoration.dbid}','','您确定取消加装信息吗？确定系统将删除已经加装信息！')"><i class="icon-remove icon-white"></i>取消加装</a>
					</c:if>
