...
<div class="location">
	...
	<a href="javascript:void(-1);" onclick="">经销商卖车</a>
</div>
 <!--location end-->
<div class="line"></div>
<div class="listOperate">
	<div class="operate">
		<%-- <a class="but butSave" href="javascript:void(-)" onclick="window.location.href='${ctx }/orderBuySalerExpress/queryList'">结算单</a> --%>
   	</div>
  	<div class="seracrhOperate">
  		<form name="searchPageForm" id="searchPageForm"  action="${ctx}/orderBuySaler/querySaleList" method="post" >
		...
		<tr >
			...
			<td>
			 ...
			 <td style="text-align: left;">
				<c:if test="${orderBuySaler.statementStatus==1 }">
					...
					<c:if test="${orderBuySaler.confireStatus==2 }">
						<a href="${ctx}/orderBuySalerExpress/edit?orderBuySalerDbid=${orderBuySaler.dbid}" class="aedit" >填写单据</a>
					</c:if>
				</c:if>
			 ...
			 <td>
				...
				<c:if test="${orderBuySaler.confireStatus==2 }">
					<span style="color: green;">同意申请</span>
				</c:if>
				<c:if test="${orderBuySaler.confireStatus==3 }">
					<span style="color: red;">驳回申请</span>
				</c:if>
			...
			<td style="text-align: center;">
			<c:if test="${orderBuySaler.confireStatus==1 }">
				<a href="javascript:void(-1)" class="aedit"  onclick="$.utile.openDialog('${ctx}/orderBuySaler/saleConfirm?dbid=${orderBuySaler.dbid }','交车绑定确认',920,500)">确认</a>|
			</c:if>
			<c:if test="${orderBuySaler.confireStatus==2 }">
				<c:if test="${orderBuySaler.sendCarStatus==1 }">
					<a href="javascript:void(-1)" class="aedit"  onclick="$.utile.openDialog('${ctx}/orderBuySaler/sendCar?dbid=${orderBuySaler.dbid }','登记出库',920,500)">登记出库</a>|
				</c:if>
			 </c:if>
			 <a href="${ctx }/orderBuySaler/view?dbid=${orderBuySaler.dbid}" class="aedit" >查看</a>
			 <c:if test="${orderBuySaler.acceptStatus==2 }">
					<c:if test="${orderBuySaler.applayTurnBackStatus==2 }">
						|<a href="javascript:void(-1)" class="aedit"  onclick="$.utile.openDialog('${ctx}/orderBuySaler/approvalApply?dbid=${orderBuySaler.dbid }','退车审批',920,500)">退车审批</a>
					</c:if>
				</c:if>
			</td>
		</tr>
