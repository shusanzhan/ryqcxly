...
<div class="location">
	...
	<a href="javascript:void(-1);" onclick="">待付款</a>
</div>
 <!--location end-->
<div class="line"></div>
<div class="listOperate">
	<div class="operate">
  	...
  	<div class="seracrhOperate">
  		<form name="searchPageForm" id="searchPageForm"  action="${ctx}/orderBuySalerExpress/queryBuyList" method="post" >
  				...
  				<tr>
	  				...
	  				<td>
	  					<select class="text small" id="statementStatus" name="statementStatus"  onchange="$('#searchPageForm')[0].submit()">
							<option value="0" >请选择...</option>
							<option value="1" ${param.statementStatus==1?'selected="selected"':'' } >待核算</option>
	  				...
	  				<td>
	  					<select class="text small" id="approvalStatus" name="approvalStatus"  onchange="$('#searchPageForm')[0].submit()">
							<option value="0" >请选择...</option>
							<option value="1" ${param.approvalStatus==1?'selected="selected"':'' } >待审批</option>
							<option value="2" ${param.approvalStatus==2?'selected="selected"':'' } >审批同意</option>
							<option value="3" ${param.approvalStatus==3?'selected="selected"':'' } >审批驳回</option>
						</select>
	  				</td>
  					<td><label>收车状态：</label></td>
	  				<td>
	  					<select class="text small" id="payStatus" name="payStatus"  onchange="$('#searchPageForm')[0].submit()">
							<option value="0" >请选择...</option>
							<option value="1" ${param.payStatus==1?'selected="selected"':'' } >等待付款</option>
							<option value="2" ${param.payStatus==2?'selected="selected"':'' } >已打款</option>
							<option value="3" ${param.payStatus==3?'selected="selected"':'' } >已收款</option>
						</select>
	  				</td>
...
<table width="100%" border="0" class="mainTable" cellpadding="0" cellspacing="0">
	<thead>
		<tr>
			...
			<td style="width:60px;">收款公司</td>
			<td style="width:60px;">结算金额</td>
			<td style="width:60px;">交易状态</td>
			<td style="width:60px;">审核状态</td>
			<td style="width:60px;">打款状态</td>
			<td style="width: 120px;">操作</td>
		</tr>
	</thead>
	<tbody>
		<c:forEach items="${page.result }" var="orderBuySalerExpress">
		<c:set value="${orderBuySalerExpress.orderBuySaler}" var="orderBuySaler"></c:set>
		...
		<tr >
			...
			<td>
				<span style="color: red;font-size: 14px;">${orderBuySalerExpress.totalAmountPrice }</span>
			</td>
			 <td>
				<c:if test="${orderBuySalerExpress.statementStatus==1 }">
					<span style="color: red;">待核单</span><br>
				</c:if>
				<c:if test="${orderBuySalerExpress.statementStatus==2 }">
					<span style="color: red;">待结算</span><br>
				</c:if>
				<c:if test="${orderBuySalerExpress.statementStatus==3 }">
					<span style="color: green;">已结算</span><br>
				</c:if>
			</td>
			<td>
			<c:if test="${orderBuySalerExpress.approvalStatus==1 }">
				...
				<c:if test="${orderBuySalerExpress.approvalStatus==2 }">
			 ...
			 <td>
				<c:if test="${orderBuySalerExpress.payStatus==1 }">
					<span style="color: red;">待打款</span><br>
				</c:if>
				<c:if test="${orderBuySalerExpress.payStatus==2 }">
					<span style="color: green;">已打款</span><br>
				</c:if>
				<c:if test="${orderBuySalerExpress.payStatus==3 }">
					<span style="color: green;">对方已收款</span>
				</c:if>
			</td>
			<td style="text-align: center;">
				<c:if test="${orderBuySalerExpress.approvalStatus!=2 }">
					<a href="${ctx}/orderBuySalerExpress/approval?orderBuySalerDbid=${orderBuySaler.dbid}" class="aedit" >审核单据</a>|
				</c:if>
				<c:if test="${orderBuySalerExpress.approvalStatus==2 }">
					<c:if test="${orderBuySalerExpress.payStatus==1 }">
						<a href="${ctx}/orderBuySalerExpress/payConfirm?orderBuySalerDbid=${orderBuySaler.dbid}" class="aedit" >确认打款</a>|
					</c:if>
				</c:if>
				<c:if test="${orderBuySalerExpress.approvalStatus==2 }">
					<a href="${ctx}/orderBuySalerExpress/print?orderBuySalerDbid=${orderBuySaler.dbid}" class="aedit" >打印</a>|
				</c:if>
				<a href="${ctx}/orderBuySalerExpress/view?orderBuySalerDbid=${orderBuySaler.dbid}" class="aedit" >查看明细</a>
			</td>
		</tr>
		</c:forEach>
	</tbody>
