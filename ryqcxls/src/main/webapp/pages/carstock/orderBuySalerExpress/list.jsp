	...
	<tbody>
		...
		<tr >
			...
			<td style="text-align: center;">
				...
				<c:if test="${orderBuySaler.statementStatus==2 }">
						<a href="${ctx}/orderBuySalerExpress/edit?orderBuySalerDbid=${orderBuySaler.dbid}" class="aedit" >修改单据</a>
						|
						<a href="javasript:void(-1)" class="aedit" onclick="$.utile.operatorDataByDbid('${ctx }/orderBuySalerExpress/statement?dbid=${orderBuySaler.orderBuySalerExpress.dbid}','searchPageForm','确定标记该申请为已结算？')">标记结算</a>
				</c:if>
				<c:if test="${orderBuySaler.statementStatus==3 }">
