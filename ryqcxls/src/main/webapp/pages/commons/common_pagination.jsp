...
<table id="paginationTableId" class="pagebar"  border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    总数${page.totalCount}
    每页显示<select name="pageSize" align="absmiddle" onchange="cwSearchPagingOrder('${page.currentPageNo}',$(this).find('option:selected').text());" id="pageSize" style="width: 50px">
    ...
    <c:otherwise>
     ...
     第
     <input type="text" onkeyup="this.value=this.value.replace(/\D/g,'');document.all.currentPage.value=this.value;" onafterpaste="this.value=this.value.replace(/\D/g,'');"  maxlength="5" name="curPage" id="curPage"  value="" size="3" style="width:27px"/>页
