...
<div class="listOperate">
	<div class="operate">
		<a class="but button" href="javascript:void();" onclick="window.location.href='${ctx}/couponMemberTemplate/add'">添加</a>
		<c:if test="${sessionScope.user.userId=='admin' }">
			<a class="but button" href="javascript:void();" onclick="$.utile.deleteIds('${ctx }/couponMemberTemplate/delete','searchPageForm')">删除</a>
		</c:if>
   </div>
  	<div class="seracrhOperate">
   		<form name="searchPageForm" id="searchPageForm" action="${ctx}/couponMemberTemplate/queryList" method="post">
...
<table width="100%"  cellpadding="0" cellspacing="0" class="mainTable" border="0">
	...
	<c:forEach var="couponMemberTemplate" items="${page.result }">
			<tr>
				<td style="text-align: center;">
						<input type="checkbox"   name="id" id="id1" value="${couponMemberTemplate.dbid }">
				</td>
				<td>${couponMemberTemplate.name }</td>
				<td style="text-align: center;">
					<c:if test="${couponMemberTemplate.type==1 }">
						折扣券							
					</c:if>
					<c:if test="${couponMemberTemplate.type==2 }">
						代金券
					</c:if>
				</td>
				<td align="center" style="text-align: center;">
					<c:if test="${couponMemberTemplate.showHiden==true }" var="status">
				...
				<td align="center" style="text-align: center;">
					<c:if test="${couponMemberTemplate.enabled==true }" var="status">
						<span style="color: blue;">是</span>
					</c:if>
					<c:if test="${status==false }">
						<span style="color: red;">否</span>
				...
				<td style="text-align: center;">
					<a href="javascript:void(-1)" class="aedit" onclick="window.location.href='${ctx}/couponMemberTemplate/edit?dbid=${couponMemberTemplate.dbid}'">编辑</a> |
					<a href="javascript:void(-1)" class="aedit" onclick="$.utile.deleteById('${ctx}/couponMemberTemplate/delete?dbids=${couponMemberTemplate.dbid}','searchPageForm')" title="删除">删除</a>
				</td>
			</tr>
	</c:forEach>
</table>
