...
<div class="listOperate">
	<div class="operate">
		...
		<a class="but butSave"  href="javascript:void(-1)" onclick="window.location.href='${ctx }/carModelSalePolicy/redBagUpdateBatch'">批量设置</a>
   </div>
  	<div class="seracrhOperate">
  		<form name="searchPageForm" id="searchPageForm" action="${ctx}/carModelSalePolicy/queryRedBagList" method="post">
	...
	<tbody>
		<c:forEach items="${page.result }" var="carModelSalePolicy">
			<tr>
				...
				<td>
					<c:if test="${carModelSalePolicy.ageRewardStatus==2}">
						<span style="color: red;">停用</span>
					</c:if>
					<c:if test="${carModelSalePolicy.ageRewardStatus==1}">
						<span style="color: green;">启用</span>
					</c:if>
				</td>
				<td>
					<c:if test="${carModelSalePolicy.finRewardStatus==2}">
						<span style="color: red;">停用</span>
					</c:if>
					<c:if test="${carModelSalePolicy.finRewardStatus==1}">
						<span style="color: green;">启用</span>
					</c:if>
				</td>
				<td>
					<c:if test="${carModelSalePolicy.carModelRewarStatus==2}">
						<span style="color: red;">停用</span>
					</c:if>
					<c:if test="${carModelSalePolicy.carModelRewarStatus==1}">
						<span style="color: green;">启用</span>【${carModelSalePolicy.carModelPrice }】
					</c:if>
				</td>
			</tr>
		</c:forEach>
