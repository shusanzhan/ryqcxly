...
<div class="listOperate">
	<div class="operate">
		...
		<a class="but button" href="javascript:void();" onclick="exportExcel('searchPageForm')">导出数据</a>
   </div>
  	<div class="seracrhOperate">
  		<form name="searchPageForm" id="searchPageForm"  action="${ctx}/netTwoCustomer/gatherCustomerFile" method="post" >
		...
		<table cellpadding="0" cellspacing="0" class="searchTable" >
  			<tr>
  				...
  				<td>
  					<select class="text small" id="distributorId" name="distributorId"  onchange="$('#searchPageForm')[0].submit()">
						<option value="0" >请选择...</option>
						<c:forEach var="distributor" items="${distributors }">
							<option value="${distributor.dbid }" ${param.distributorId==distributor.dbid?'selected="selected"':'' } >${distributor.shortName }</option>
						</c:forEach>
					</select>
				</td>
  			...
  			<tr>
  				...
  				<td><input type="text" id="mobilePhone" name="vinCode" class="text small" value="${param.vinCode}"></input></td>
...
<script type="text/javascript">
	function fn(va){
	 ...
	 function exportExcel(searchFrm){
	 	...
	 	window.location.href='${ctx}/netTwoCustomer/exportExcel?'+params;
	 }
</script>
</html>
