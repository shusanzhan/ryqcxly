...
<div class="row-fluid" >
	  	<div class="seracrhOperate" style="float: left;">
	  		<form name="searchPageForm" id="searchPageForm"  action="${ctx}/statistics/carStatistics" method="post" >
...
<div class="row-fluid">
	<table class="table table-bordered table-striped">
		<thead>
			<tr>
				<th>车型分析</th>
				<c:forEach var="mapCarse" items="${mapCarser }">
					<c:if test="${mapCarse.key==1 }">
						<th colspan="${mapCarse.value }">主力车型</th>
					</c:if>
					<c:if test="${mapCarse.key==2 }">
						<th colspan="${mapCarse.value }">发力车型</th>
					</c:if>
					<c:if test="${mapCarse.key==3 }">
						<th colspan="${mapCarse.value }">清库车型</th>
					</c:if>
					<c:if test="${mapCarse.key==4 }">
						<th colspan="${mapCarse.value }">其他</th>
					</c:if>
				</c:forEach>
				<th></th>
			</tr>
