...
<body class="bodycolor" style="">
	...
	<form action="" name="frmId" id="frmId"  target="_self">
		...
		<table border="0" align="center" cellpadding="0" cellspacing="0" style="width: 92%;">
			...
			<tr height="42">
				...
				<td ><select class="select text" id="tarBussiStaff" name="tarBussiStaff" checkType="integer,1" tip="请选择目标销售人员！">
						<option value="">请选择...</option>
						<option value="${sessionScope.user.dbid }">${sessionScope.user.realName }</option>
						<c:forEach var="user" items="${users }">
							<option value="${user.dbid }" >${user.realName }</option>
						</c:forEach>
					</select>
		...
		<div class="formButton">
			...
			<a href="javascript:void()"	onclick="$('#type').val(4);$.utile.submitAjaxForm('frmId','${ctx}/turnCustomerRecord/save')"	class="but butSave">保存并继续</a> 
