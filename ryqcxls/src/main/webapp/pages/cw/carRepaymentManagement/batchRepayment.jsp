...
<div class="location">
	...
	<a href="javascript:void(-1);" onclick="window.location.href='${ctx}/carRepaymentManagement/queryFinBillPayment'">金融还款</a>-
	<a href="javascript:void(-1);" onclick="javascript:void(-1);">批量支出</a>
</div>
<div class="line"></div>
<div class="frmContent">
		<form action="" name="frmId1" id="frmId1" >
	<div class="frmTitle" onclick="showOrHiden('contactTable')">待批量还款车辆</div>
	<div>
	<c:set value="0" var="totalMoney"></c:set>
	<c:forEach var="factoryOrder" items="${factoryOrders }" >
		<span class="cust" style="">${factoryOrder.vinCode }【￥${factoryOrder.presaleRebate }】</span>
		<c:set value="${factoryOrder.presaleRebate+totalMoney }" var="totalMoney"></c:set>
		<input type="hidden" name="id" value="${factoryOrder.dbid}">
	</c:forEach>
	...
	<tr>
		<td class="formTableLeft">支出总额：&nbsp;</td>
		<td colspan="4">
	...
	<div class="formButton">
	    <a id="correct" href="javascript:void(-1)"	 onclick="$.utile.submitFrm('frmId1','${ctx}/carRepaymentManagement/saveBatchRepayment')"	class="but butSave dis">确定支出（￥${totalMoney }）</a> 
