...
<div class="location">
	...
	<a href="javascript:void(-1);" onclick="window.location.href='${ctx}/cashFinance/queryBatchDiscountCashList'">金融贴息批量收银</a>-
...
<div class="frmContent">
	...
	<c:set value="0" var="totalMoney"></c:set>
		<c:forEach var="finCustomer" items="${finCustomers }" >
			<span class="cust" style="">${finCustomer.name }【￥${finCustomer.finCustomerLoan.discountMony }】</span>
			<c:set value="${finCustomer.finCustomerLoan.discountMony+totalMoney }" var="totalMoney"></c:set>
			<input type="hidden" name="finCustomerIds" value="${finCustomer.dbid}">
		</c:forEach>
	...
	<tr>
		...
		<td>
			<input type="text"  readonly="readonly" id="amountCollected"  value="${totalMoney}" class="largeX text" ></input>
			<span style="color: red">*</span>
		</td>
		<td class="formTableTdLeft">实收总额：&nbsp;</td>
			<td>
					<input type="text"  name="cashier.amountCollected"  onkeyup="collectionIsCorrect1(this)"  onfocus="collectionIsCorrect1(this)"  id="totalMoney"   class="largeX text" value="${totalMoney}" checkType="float,0" tip="实收总额不能为空，且为数字类型"></input>
					<span style="color: red">*</span>
			</td>
	</tr>
		<tr>
	...
	<div class="formButton">
		<a id="correct" href="javascript:void(-1)"	 onclick="$.utile.submitFrm('frmId1','${ctx}/cashFinance/saveBatchCash')"	class="but butSave dis">确定收银（￥${totalMoney }）</a> 
