...
<div class="location">
		...
		<a href="javascript:void(-1);" onclick="javascript:void(-1);">收银</a>
</div>
<div class="line"></div>
<div class="frmContent">
		...
		<table border="0" align="center" cellpadding="0" cellspacing="0" style="width: 98%;">
			...
			<tr height="42">
				...
				<td  colspan="3">
					...
					<c:if test="${empty finCustomer.distributor.name }">
						无
					</c:if>
					<c:if test="${!empty finCustomer.distributor.name}">
						${finCustomer.distributor.name }
		...
		<tr height="42">
			...
			<td colspan="3" id="amountCollected" style="color: red;font-size: 18px;">${finCustomer.finCustomerLoan.loanPrice}</td>
		</tr>
		</table>	
	...
	<table border="0" align="center" cellpadding="0" cellspacing="0"  style="width:98%">
		...
		<td class="formTableTdLeft">实收总额：&nbsp;</td>
			<td >
					<input type="text"  name="cashier.amountCollected"  id="totalMoney"  class="largeX text"  value="${finCustomer.finCustomerLoan.loanPrice}" onkeyup="totalCollection();$('#openBillingMoney').val(this.value),collectionIsCorrect1(this)" onfocus="totalCollection(),collectionIsCorrect1(this)" checkType="float,0" tip="实收总额不能为空，且为数字类型"></input>
					<span style="color: red">*</span>
			</td>
	...
	<div class="formButton">
		<a id="correct" href="javascript:void(-1)"	 onclick="$.utile.submitFrm('frmId','${ctx}/cashFinance/save')"	class="but butSave dis">确定收银（￥${finCustomer.finCustomerLoan.loanPrice}）</a> 
