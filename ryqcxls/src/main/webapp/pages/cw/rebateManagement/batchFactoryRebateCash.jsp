...
<div class="location">
	...
	<a href="javascript:void(-1);" onclick="window.location.href='${ctx}/rebateManagement/queryFactoryRebateList	'">工厂返利收银</a>-
	<a href="javascript:void(-1);" onclick="javascript:void(-1);">批量收银</a>
</div>
<div class="line"></div>
<div class="frmContent">
	<form action="" name="frmId1" id="frmId1" >
	<div class="frmTitle" onclick="showOrHiden('contactTable')">待批量收银返利</div>
	<div>
		<c:set value="0" var="totalMoney"></c:set>
		<c:forEach var="rebate" items="${rebates }" >
			<span class="cust" style="">${rebate.factoryOrder.vinCode }(${rebate.name })【￥${rebate.rebateMoney }】</span>
			<c:set value="${rebate.rebateMoney+totalMoney }" var="totalMoney"></c:set>
			<input type="hidden" name="rebateId" value="${rebate.dbid}">
		</c:forEach>
	...
	<tr>
		...
		<td class="formTableTdLeft">实收总额：&nbsp;</td>
			<td>
					<input type="text"  name="cashier.amountCollected"  id="totalMoney"  class="largeX text" value="${totalMoney}" onkeyup="collectionIsCorrrect1(this)" onfocus="collectionIsCorrrect1(this)"></input>
					<span style="color: red">*</span>
			</td>
	</tr>
		<tr>
	...
	<div class="formButton">
		<a id="correct" href="javascript:void(-1)"	 onclick="$.utile.submitFrm('frmId1','${ctx}/rebateManagement/saveBatchFactoryRebateCash')"	class="but butSave dis">确定收银（￥${totalMoney }）</a> 
