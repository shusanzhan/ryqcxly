...
<div class="location">
	...
	<a href="javascript:void(-1);" onclick="">返利售前项目记录</a>
</div>
 <!--location end-->
<div class="line"></div>
<div class="listOperate">
  	<div class="seracrhOperate">
  		<form name="searchPageForm" id="searchPageForm"  action="${ctx}/rebateManagement/queryBeforeRebateRecordList" method="post" >
		...
		<table cellpadding="0" cellspacing="0" class="searchTable" >
  			<tr>
  				...
  				<td><label>车辆付款状态：</label></td>
  				<td>
  					<select class="text middle" id="costPayment" name="costPayment" onchange="$('#searchPageForm')[0].submit()">
  						<option value="">请选择...</option>
  						<option value="1" ${param.costPayment==1?'selected="selected"':''}>默认</option>
  						<option value="2" ${param.costPayment==2?'selected="selected"':''}>待付</option>
  						<option value="3" ${param.costPayment==3?'selected="selected"':''}>已付</option>
  					</select>
  				</td>
  			</tr>
  			<tr>
  				<td><label>VIN码：</label></td>
  				<td><input type="text" id="vinCode" name="vinCode" class="text middle" value="${param.vinCode }"></td>
...
<c:if test="${!empty(page.result)}" var="status">
	...
	<thead>
		<tr>
			...
			<td style="width: 70px;">返利名称</td>
			<td style="width: 70px;">工厂价</td>
	...
	<tbody>
		...
		<tr >
			...
			<td>
				<c:if test="${rebate.factoryOrder.costPayment eq 1}">
					默认
				</c:if>
				<c:if test="${rebate.factoryOrder.costPayment eq 2}">
					<span style="color:red">待付</span>
				</c:if>
				<c:if test="${rebate.factoryOrder.costPayment eq 3}">
			...
			<td style="text-align: center;">
				<c:if test="${rebate.factoryOrder.costPayment eq 1}">
					<a href="${ctx}/rebateManagement/editBeforeRebateRecord?rebateId=${rebate.dbid}" style="color:#2b7dbc" class="aedit">编辑返利</a>
				</c:if>
				<c:if test="${rebate.factoryOrder.costPayment eq 2}">
					<a href="${ctx}/rebateManagement/editBeforeRebateRecord?rebateId=${rebate.dbid}" style="color:#2b7dbc" class="aedit">编辑返利</a>
				</c:if>
				<c:if test="${rebate.factoryOrder.costPayment eq 3}">
					<a href="${ctx}/rebateManagement/editBeforeRebateRecord?rebateId=${rebate.dbid}" class="aedit" style="color:#2b7dbc ">查看返利</a>
				</c:if>
			</td>
		</tr>
		</c:forEach>
