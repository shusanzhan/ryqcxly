...
<div class="listOperate">
	<div class="operate">
		<a class="but butCancle" href="javascript:void(-1);" onclick="$.utile.operatorDataByDbids('${ctx }/finCustomer/cancelCheckCustomer','searchPageForm','您确定批量撤销选中客户吗')">批量撤销</a>
   </div>
  	<div class="seracrhOperate">
  		<form name="searchPageForm" id="searchPageForm" action="${ctx}/finCustomer/queryCheckFinCustomerList" method="post">
			...
			<table cellpadding="0" cellspacing="0" class="searchTable" >
				<tr>
					<td><label>客户类型：</label></td>
	  				...
	  				<td><label>核单状态：</label></td>
	  				<td>
	  					<select class="small text" id="checkStatus" name="checkStatus" onchange="$('#searchPageForm')[0].submit()" >
							<option value="">请选择...</option>
							<option value="1" ${param.checkStatus==1?'selected="selected"':'' } >待核单</option>
							<option value="2" ${param.checkStatus==2?'selected="selected"':'' } >已核单</option>
						</select>
					</td>
	   			...
	   			<tr>
					...
					<tr>
						...
						<td><label>核查日期：</label></td>
		  				<td>
		  					<input type="text" id="checkStartDate" name="checkStartDate" class="text small" value="${param.checkStartDate}" onfocus="WdatePicker()"></input>
		  				</td>
		  				<td style="text-align:center; "><label>~</label></td>
		  				<td>
		  					<input type="text" id="checkEndDate" name="checkEndDate" class="text small" value="${param.checkEndDate}" onfocus="WdatePicker()"></input>
						</td> 
		  			</tr>
		  			<tr>
		  				<td>
		...
		<tr height="32" align="center">
					...
					<div class="drop_down_menu hiden" onmousemove="show(this)" onmouseout="hi(this)" >
					    <ul>
					      ...
					      <c:if test="${finCustomer.applyStatus==2 }">
						      ...
						      <li><a href="#" class="aedit" onclick="$.utile.openDialog('${ctx}/finCustomer/toFinCustomerFile?finCustomerId=${finCustomer.dbid }&type=2','客户归档',720,400)">客户归档</a></li>
					      </c:if>
					    </ul>
					  </div>
				</div>
			...
			<td style="text-align: center;">
					...
					<c:if test="${finCustomer.checkStatus==2 }">
						<span style="color:green;">已经审核</span>
						<br><fmt:formatDate value="${finCustomer.checkDate }"/> 
					</c:if>
			</td>
			<td style="text-align: center;">
			...
			<td>
				<c:if test="${finCustomer.checkStatus==1}">
					<a href="#" class="aedit" onclick="window.location.href='${ctx }/finCustomer/checkCustomer?dbid=${finCustomer.dbid}&type=1'">核单</a>
				</c:if>
				<c:if test="${finCustomer.checkStatus==2}">
					<a href="#" class="aedit" onclick="$.utile.operatorDataByDbid('${ctx }/finCustomer/cancelCheckCustomer?dbids=${finCustomer.dbid}','searchPageForm','您确定【${finCustomer.name}】撤销客户吗')">撤销</a>
				</c:if>
				<c:if test="${finCustomer.applyStatus==2 }">
					|
					<a href="#" class="aedit" onclick="window.location.href='${ctx }/finCustomer/updateFinCustomer?dbid=${finCustomer.dbid}&type=2'">更新业务</a>
				</c:if>
		</tr>
	</c:forEach>
</table>
