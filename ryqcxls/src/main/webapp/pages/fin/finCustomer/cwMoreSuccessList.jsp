...
<div class="listOperate">
	<div class="operate">
		<a class="but button" href="javascript:void();" onclick="frozenMoreCwCustomer()">批量冻结</a>
   </div>
  	<div class="seracrhOperate">
  		<form name="searchPageForm" id="searchPageForm" action="${ctx}/finCustomer/queryCwMoreSuccessList" method="post">
		<input type="hidden" id="currentPage" name="currentPage" value='${page.currentPageNo}'>
		<input type="hidden" id="paramPageSize" name="pageSize" value='${page.pageSize}'>
			<table cellpadding="0" cellspacing="0" class="searchTable" >
				<tr>
					<td><label>提报状态：</label></td>
	  				<td>
	  					<select class="text small" id="cwStatus" name="cwStatus"  onchange="$('#searchPageForm')[0].submit()">
							<option value="0" >请选择...</option>
							<option value="1" ${param.cwStatus==1?'selected="selected"':'' } >待提报</option>
							<option value="2" ${param.cwStatus==2?'selected="selected"':'' } >已提报</option>
						</select>
	  				</td>
...
<table width="100%"  cellpadding="0" cellspacing="0" class="mainTable" border="0">
	...
	<c:forEach var="finCustomer" items="${page.result }">
		<tr height="32" align="center">
			...
			<td>
				<c:if test="${finCustomer.frozenStatus==2 }">
					<a href="#" class="aedit" onclick="$.utile.operatorDataByDbid('${ctx }/cwCustomer/frozenCwCustomer?dbid=${finCustomer.dbid}','searchPageForm','您确定【${finCustomer.name }】解冻该客户吗')">解冻</a>
				</c:if>
				<c:if test="${finCustomer.frozenStatus==1 }">
					<a href="#" class="aedit" onclick="$.utile.operatorDataByDbid('${ctx }/cwCustomer/frozenCwCustomer?dbid=${finCustomer.dbid}','searchPageForm','您确定【${finCustomer.name }】冻结该客户吗')">冻结</a>
				</c:if>
				<c:if test="${finCustomer.cwStatus==2 }">
					<c:if test="${finCustomer.cwAppStatus==2 }">
						<a href="javascript:void(-1)" class="aedit" onclick="$.utile.openDialog('${ctx}/cwAppmodifydata/finApproval?finCustomerId=${finCustomer.dbid }','审批金融修改资料申请',720,400)">审批</a>
					</c:if>
				</c:if>
			</td>
