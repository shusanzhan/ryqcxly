...
<div id="detail_nav">
	<div class="detail_nav_inner">
	      <ul class="clearfix padding10">
	        ...
	        <li onclick="window.location.href='${ctx }/visitRecordReport/monthReportUser?brandId=${brandId}&monthDate=${monthDate }'" id="pingjia_tap" class="detail_tap5 detail_tap pull_left select">满意度部门负责人</li>
...
<table width="100%"  cellpadding="0" cellspacing="0" class="mainTable" border="0">
	...
	<c:forEach var="visitRecordReportItem" items="${visitRecordReportUsers}" varStatus="i">
		<tr height="32" align="center">
			<td>${i.index+1 }</td>
			<td><a href="javascript:void(-1)" class="aedit" style="color: green" onclick="$.utile.openDialog('${ctx }/visitRecordReport/monthReportDepManager?managerId=${visitRecordReportItem.userId }&brandId=${brandId}&monthDate=${monthDate }','${visitRecordReportItem.userName }负责部门',1020,420);">${visitRecordReportItem.userName }</a></td>
			...
			<td>
				<ystech:visitRecordReportManagerTag monthDate="${monthDate }" brandId="${brandId }" mangerId="${visitRecordReportItem.userId }"/>
			</td>
		</tr>
	</c:forEach>
</table>
