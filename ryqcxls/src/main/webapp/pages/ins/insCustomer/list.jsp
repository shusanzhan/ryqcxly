...
<div class="listOperate">
	<div class="operate">
		<a class="but button" href="javascript:void();" onclick="window.location.href='${ctx}/insuranceRecord/addOutInsCustomer?type=1&insType=3'">新增客户购买</a>
   </div>
  	<div class="seracrhOperate">
  		<form name="searchPageForm" id="searchPageForm" action="${ctx}/insCustomer/queryList" method="post">
...
<table width="100%"  cellpadding="0" cellspacing="0" class="mainTable" border="0">
	<thead  class="TableHeader">
		<tr>
			...
			<td class="span2">历史返利</td>
			<td class="span2">历史权益</td>
	...
	<c:forEach var="insCustomer" items="${page.result }">
		<tr height="32" align="center">
			...
			<td style="text-align: center;">
				${insCustomer.historyRebateMoney }
			</td>
			<td style="text-align: center;">
				${insCustomer.historyIncidentalInterestMoney }
			</td>
			
			<td>
				<c:if test="${insCustomer.buyInsuranceStatus==2 }">
					<c:if test="${insCustomer.customerType==1 }">
						<a href="#" class="aedit" onclick="window.location.href='${ctx }/insuranceRecord/add?dbid=${insCustomer.dbid}&type=1&insType=2'">续保</a>
					</c:if>
					<c:if test="${insCustomer.customerType==2 }">
						<a href="#" class="aedit" onclick="window.location.href='${ctx }/insuranceRecord/add?dbid=${insCustomer.dbid}&type=1&insType=4'">续保</a>
					</c:if>
				</c:if>
				<c:if test="${insCustomer.customerType==1&&insCustomer.buyInsuranceStatus==1}">
					<a href="#" class="aedit" onclick="window.location.href='${ctx }/insuranceRecord/add?dbid=${insCustomer.dbid}&type=1&insType=1'">购买</a>
				|	<a href="javascript:void(-1)" class="aedit" onclick="$.utile.openDialog('${ctx}/insCustomer/outFlow?dbid=${insCustomer.dbid }','客户流失记录',760,440)">流失</a>
				</c:if>
		</tr>
	</c:forEach>
</table>
