...
<div class="frmContent">
	<form action="" name="frmId" id="frmId"  target="_self">
		...
		<table border="0" align="center" cellpadding="0" cellspacing="0" style="width: 100%;">
			...
			<tr height="42">
				...
				<td >
					<c:if test="${!empty(insCustomer.brand) }">
			...
			<tr height="32">
				...
				<td>
					<c:if test="${empty(insCustomer.carPlateNo) }">
						<input id="carPlateNo" class="large text"   name="carPlateNo" value="${insCustomer.carPlateNo }"  checkType="string,5" canEmpty="Y" tip="车牌号不能为空">
		...
		<table border="0" align="center" cellpadding="0" cellspacing="0" style="width: 100%;margin-top: 5px;">
				...
				<tr style="height: 40px;" height="40">
					...
					<td colspan="" >
						...
						<input type="hidden" name="nonDeductiblePriceTotal"  id="nonDeductiblePriceTotal" value="${insuranceRecord.nonDeductiblePrice}">
				...
				<tr style="height: 40px;" height="40">
					...
					<td colspan="" >
						<input id="strongRiskPrice" class="large text"   name="insuranceRecord.strongRiskPrice" value="${insuranceRecord.strongRiskPrice }" onkeyup="getTotalPrice()" onfocus="getTotalPrice()" checkType="float,0" tip="强制险险不能为空">
						<span style="color: red;">*</span>
					</td>
				...
				<tr style="height: 40px;" height="40">
					<td class="formTableTdLeft" width="120">强制险返利:&nbsp;</td>
					<td colspan="" >
						<input id="strongRiskRebateMoney" class="large text" readonly="readonly"  name="insuranceRecord.strongRiskRebateMoney" value="${insuranceRecord.strongRiskRebateMoney }" onkeyup="getTotalPrice()" checkType="float,0"  tip="强制险不能为空">
						<span style="color: red;">*</span>
					</td>
					<td class="formTableTdLeft">商业险返利:&nbsp;</td>
					<td >
						<input type="text" readonly="readonly"   name="insuranceRecord.busiRiskRebateMoney" id=busiRiskRebateMoney	value="${insuranceRecord.busiRiskRebateMoney}" class="large text"  checkType="float,0"  tip="商业险返利">
						<span style="color: red;">*</span>
					</td>
				</tr>
				<tr style="height: 40px;" height="40">
					<td class="formTableTdLeft" width="120">返利总额:&nbsp;</td>
					<td colspan="" style="font-size: 10px;">
						<input id="rebateMoney" readonly="readonly"   class="large text"   name="insuranceRecord.rebateMoney" value="${insuranceRecord.rebateMoney}" onkeyup="getTotalPrice()" checkType="float,0"  tip="强制险不能为空">
						<span style="color: red;">*</span><br>
						返利合计:<span id="strongRiskPerText">${insuranceRecord.strongRiskPrice}*${insuranceRecord.strongRiskPer }%</span>+<span id="busiRiskPerText">${insuranceRecord.busiRiskPrice}*${insuranceRecord.busiRiskPer }%</span>=<span style="font-size: 20px;color: red;margin:0 5px;" id="rebateMoneyText">${insuranceRecord.rebateMoney }</span>元
					</td>
					<td class="formTableTdLeft" width="120">客户权益:&nbsp;</td>
					<td colspan="" >
						<input type="text" readonly="readonly" name="insuranceRecord.incidentalInterestMoney"  id="incidentalInterestMoney" value="${insuranceRecord.incidentalInterestMoney }" class="large text">
						<span style="color: red;">*</span>
					</td>
				</tr>
					<tr style="height: 40px;${insCustomer.customerType==1?'display:;':'display: none;'}" height="40">
						<td class="formTableTdLeft">销售提成比例:&nbsp;</td>
						<td >
							<c:if test="${empty(insuranceRecord.salerBasePercent) }">
								<input type="text" readonly="readonly" name="insuranceRecord.salerBasePercent"  id="salerBasePercent" value="${100-insSet.salerBasePrice }" class="large text">%
							</c:if>
							<c:if test="${!empty(insuranceRecord.salerBasePercent) }">
								<input type="text" readonly="readonly" name="insuranceRecord.salerBasePercent"  id="salerBasePercent" value="${insuranceRecord.salerBasePercent }" class="large text">%
							</c:if>
							<span style="color: red;">*</span>
						</td>
						<td class="formTableTdLeft">提成基数:&nbsp;</td>
						<td >
							<input type="text" readonly="readonly" name="insuranceRecord.salerBasePrice" id="salerBasePrice"	value="${insuranceRecord.salerBasePrice }" class="large text" onkeyup="getTotalPrice()" checkType="float,0"  tip="提成基数不能为空">
							<span style="color: red;">*</span>
							<br>(返利总额-客户权益)*（${100-insSet.salerBasePrice}%)
						</td>
					</tr>
				<tr style="height: 40px;" height="40">
					<td class="formTableTdLeft">销售人员:&nbsp;</td>
					<td colspan="1"><input type="text" class="large text"  name="insuranceRecord.saler" id="saler" onfocus="autoUser('saler')" 	value="${insuranceRecord.saler}" checkType="string,1,50"  tip="起保日期日期不能为空"></td>
			...
			<table border="0" align="center" cellpadding="0" cellspacing="0" style="width: 100%;margin-top: 5px;">
				...
				<c:forEach var="insuranceItem" items="${bussiInsuranceItems }" varStatus="questIndex">
						...
						<tr height="40" style="height: 40px;">
							...
							<td width="200">
								<c:if test="${insuranceItem.nonDeductibleStatus==2 }">
									...
									<label>
										<input type="radio" onclick="showNonDe(${insuranceItem.dbid });jisuanNo()" name="nonDeductibleStatus${insuranceItem.dbid }" id="nonDeductibleStatus${insuranceItem.dbid }" value="1" checked="checked">否&nbsp;&nbsp;
									</label>
									<label>
										<input type="radio" onclick="showNonDe(${insuranceItem.dbid });jisuanNo();" name="nonDeductibleStatus${insuranceItem.dbid }" id="nonDeductibleStatus${insuranceItem.dbid }" value="2">是&nbsp;&nbsp;
									</label>
					...
					<c:if test="${itemStatus==true }">
						<tr height="40" style="height: 40px;">
							...
							<td width="200">
								<c:if test="${insuranceItem.nonDeductibleStatus==2 }">
									<input type="hidden" name="nonDeductiblePer${insuranceItem.dbid }" id="nonDeductiblePer${insuranceItem.dbid }" value="${insuranceRecordItemTarget.nonDeductiblePer }" >
									<c:if test="${insuranceRecordItemTarget.nonDeductibleStatus==1 }">
										<label>
											<input type="radio" onclick="showNonDe(${insuranceItem.dbid });jisuanNo()" name="nonDeductibleStatus${insuranceItem.dbid }" id="nonDeductibleStatus${insuranceItem.dbid }" value="1" >否&nbsp;&nbsp;
										</label>
										<label>
											<input type="radio" onclick="showNonDe(${insuranceItem.dbid });jisuanNo();" name="nonDeductibleStatus${insuranceItem.dbid }" id="nonDeductibleStatus${insuranceItem.dbid }" value="2" checked="checked">是&nbsp;&nbsp;
											${insuranceItem.nonDeductiblePer }%
										</label>
										<input type="text" id="nonDeductiblePrice${insuranceItem.dbid }" nonDeductiblePrice="bnonDeductiblePrice"  name="nonDeductiblePrice${insuranceItem.dbid }" value="" onkeyup="jisuanNo(this)" class="smallX text" style="width: 50px;display: none;">
									</c:if>
									<c:if test="${insuranceRecordItemTarget.nonDeductibleStatus==2 }">
										<label>
											<input type="radio" onclick="showNonDe(${insuranceItem.dbid });jisuanNo()" name="nonDeductibleStatus${insuranceItem.dbid }" id="nonDeductibleStatus${insuranceItem.dbid }" value="1" ${insuranceRecordItemTarget.nonDeductibleStatus==1?'checked="checked"':'' } >否&nbsp;&nbsp;
										</label>
										<label>
											<input type="radio" onclick="showNonDe(${insuranceItem.dbid });jisuanNo();" name="nonDeductibleStatus${insuranceItem.dbid }" id="nonDeductibleStatus${insuranceItem.dbid }" value="2" ${insuranceRecordItemTarget.nonDeductibleStatus==2?'checked="checked"':'' }>是&nbsp;&nbsp;
											${insuranceItem.nonDeductiblePer }%
										</label>
									</c:if>
								</c:if>
				...
				<tr height="42" style="background-color: #eeeeee; border-color:#eeeeee;font-size: 16px;font-weight: bold;">
					<td width="100%" colspan="5" style="text-align: right;padding-right: 12px;">总计：<span style="font-size: 20px;color: red;margin:0 5px;" id="totalPriceText">${insuranceRecord.totalPrice }</span>元</td>
				</tr>
			</table>
					...
					<c:if test="${empty(insCustomer) }">
						<c:forEach var="i" begin="0" step="1" end="3">
							<tr id="tr${i+1 }">
								...
								<td style="text-align: center;">
									<input type="text"  name="price" id="price${i+1 }" class="small text"   onkeyup="decorePrice();getTotalPrice();" checkType="float,1" canEmpty="Y" tip="请输入项目价值，必须为数字" style="width: 95%">
								</td>
								<td>
					...
					<c:if test="${!empty(insCustomer) }">
						...
						<c:forEach var="insuranceIncidentalInterest" items="${insuranceIncidentalInterests }" varStatus="i">
							<tr id="tr${i.index+1 }">
								...
								<td style="text-align: center;">
									<input type="text"  name="price" id="price${i.index+1 }"  class="small text"  value="${insuranceIncidentalInterest.price }" onkeyup="decorePrice();getTotalPrice();" checkType="float,1" canEmpty="Y" tip="请输入项目价值，必须为数字" style="width: 95%">
								</td>
								<td>
	...
	<div class="formButton">
		<a href="javascript:void(-1)"	onclick="$.utile.submitForm('frmId','${ctx}/insuranceRecord/save')"	class="but butSave">保&nbsp;&nbsp;存</a> 
...
function getTotalPrice(){
	...
	var incidentalInterestMoney= $("#incidentalInterestMoney").val();
	incidentalInterestMoney=parseFloat(incidentalInterestMoney);
	if(isNaN(incidentalInterestMoney)){
		incidentalInterestMoney=0;
	}
	...
	var salerBasePrice=0;
	var salerBasePercent=$("#salerBasePercent").val();
	salerBasePercent =formatFloat(salerBasePercent);
	salerBasePrice=(rebateMoney-incidentalInterestMoney)*salerBasePercent/100;
	$("#salerBasePrice").val(formatFloat(salerBasePrice));
}
//商品表格编辑部分
function create(v){
	var value=$(v).val();
...
function crateTr() {
	...
	var str = ' <tr id="tr'+size+'">'
			...
			+ '</td>'
			+ '<td  style="text-align: center;">'
				+ '<input  type="text"  name="price" id="price'+size+'" class="small text" onkeyup="decorePrice();getTotalPrice();" checkType="float,1" canEmpty="Y" tip="请输入项目价值，必须为数字" style="width: 95%;" >'
			+ '</td>'
			+ '<td style="text-align: center;">'
