...
<div class="frmContent">
		...
		<div id='te' style="width: 100%">
			<table id="shopNumber" border="0" align="center" cellpadding="0" cellspacing="0" style="width: 100%;margin-top: 5px;">
				...
				<tr height="32" style="background-color: #eeeeee; border-color:#eeeeee;font-size: 16px;font-weight: bold;">
						...
						<th style="width: 60px;text-align: center;">入库单号</th>
				...
				<s:set value="0" var="lengthNum"></s:set>
					<c:forEach var="decoreOutProduct" items="${saleDecoreOutProducts }" varStatus="i">
							<tr id="str${i.index+1}" onclick="">
								<td style="text-align: center;">
									${i.index+1}
								</td>
								<td >
									<input type="text" name="productItemNo" id="productItemNo${lengthNum}" value="${decoreOutProduct.purchaseProductNo }"  class="media text" onfocus="ajaxPurchaseProduct(${decoreOutProduct.purchaseProduct.dbid})">
									<input type="hidden" name="purchaseProductDbid" id="purchaseProductDbid" value="${decoreOutProduct.purchaseProduct.dbid }">
								...
								<td>
									<input type="text" name="note" id="note${i.index+1}" class="midea text"  checkType="string,1" canEmpty="Y" value="${decoreOutProduct.note }" tip="请输入项目备注信息" >
			...
			<%-- <tr height="42">
				...
				<td >
					<c:if test="${empty(decoreOut) }">
						<input type="text" readonly="readonly" name="decoreOut.zkl" id="zkl" onkeyup="disMoneyDis(this.value)" value="100" class="largeX text" title="折扣率" placeholder="100"	checkType="float,0" tip="折扣率不能为空">%<span style="color: red;">*</span>
					</c:if>
					<c:if test="${!empty(decoreOut) }">
						<input type="text" readonly="readonly" name="decoreOut.zkl" id="zkl" onkeyup="disMoneyDis(this.value)" value="${decoreOut.zkl }" class="largeX text" title="折扣率" placeholder="100"	checkType="float,0" tip="折扣率不能为空">%<span style="color: red;">*</span>
					</c:if>
				</td>
				<td class="formTableTdLeft">折扣后金额:&nbsp;</td>
				<td>
					<c:if test="${empty(decoreOut) }">
						<input type="text" readonly="readonly" name="decoreOut.disMoney" id="disMoney" value="0" class="largeX text" title="折扣后金额" checkType="float,0" tip="折扣后金额不能为空"><span style="color: red;">*</span>
					</c:if>
					<c:if test="${!empty(decoreOut) }">
						<input type="text" readonly="readonly" name="decoreOut.disMoney" id="disMoney" value="${decoreOut.disMoney }" class="largeX text" title="折扣后金额" checkType="float,0" tip="折扣后金额不能为空"><span style="color: red;">*</span>
					</c:if>
				</td>
			</tr> --%>
			<tr height="42">
	...
	<div class="formButton">
		<a href="javascript:void(-1)"	onclick="deleteNotDecoreOut();$.utile.submitForm('frmId','${ctx}/decoreOut/save')"	class="but butSave">保&nbsp;&nbsp;存</a> 
...
function notDecoreOut(dbid){
	$("#str"+dbid).hide();
	var copy=window.document.getElementById("str"+dbid).outerHTML;
	crateTr2(dbid);
	addDecorePrice(dbid);
}
...
function crateTr2(dbid) {
	...
	var str = ' <tr id="tr'+size+'" style="background-color: #F2DEDE;">'
			...
			+ '</td>'
			+ '<td  style="text-align: center;">'+$("#unit"+dbid).text()
			+ '</td>'
