...
<div class="frmContent">
		...
		<div id='te' style="width: 100%">
			<table id="shopNumber" border="0" align="center" cellpadding="0" cellspacing="0" style="width: 100%;margin-top: 5px;">
				<tr height="32" style="background-color: #eeeeee; border-color:#eeeeee;font-size: 16px;font-weight: bold;">
						...
						<th style="width: 60px;text-align: center;">品名规格</th>
						<th style="width: 120px;text-align: center;">数量</th>
						<th style="width: 120px;text-align: center;">进货单号</th>
				...
				<s:set value="0" var="lengthNum"></s:set>
					<c:forEach var="decoreOutProduct" items="${decoreOutProducts }" varStatus="i">
							<c:set value="${i.index }" var="lengthNum"></c:set>
							<tr id="tr${lengthNum}" onclick="">
								<td style="text-align: center;">
									${lengthNum+1}
								</td>
								<td >
									<input type="text" name="productName" id="productName${lengthNum }" onfocus="autoProductItemByNo('productName${lengthNum }');create(this)"  value="${decoreOutProduct.productName }" onclick="ajaxPurchaseProduct('purchaseProductDbid${lengthNum}')" class="media text" >
									<input type="hidden" name="decoreOutProductDbid" id="decoreOutProductDbid${lengthNum}" value="${decoreOutProduct.dbid}">
									<input type="hidden" name="productId" id="productId${lengthNum}" value="${decoreOutProduct.product.dbid }">
									<input type="hidden" name="purchaseProductDbid" id="purchaseProductDbid${lengthNum}" >
								</td>
								<td>
									<input  type="text" name="num" id="num${lengthNum }" value="${decoreOutProduct.num }"  class="small text" onfocus="decorePrice(${lengthNum })" onchange="decorePrice(${lengthNum })">
									<input  type="hidden" name="overNum" id="overNum${lengthNum }"  class="small text" >
								</td>
								<td>
									<input readonly="readonly" type="text" name="purchaseProductNo" id="purchaseProductNo${lengthNum }"  class="small text" >
								</td>
								<td style="text-align: center;">
									<input type="text"  readonly="readonly" name="unit" id="unit${lengthNum}" class="small text"  value="${decoreOutProduct.product.unit }">
								</td>
								<td style="text-align: center;">
									<input type="text"  readonly="readonly" name="productType" id="productType${lengthNum}" class="small text" value="${decoreOutProduct.product.productcategory.name }">
								</td>
								<td style="text-align: center;">
									<input type="text" readonly="readonly" name="price" id="price${lengthNum}" class="small text"   onkeyup="decorePrice(${lengthNum})" value="${decoreOutProduct.price}" checkType="float,1" canEmpty="Y" tip="请输入商品单价，必须为数字" >
								</td>
								<td style="text-align: center;">
									<input type="hidden" readonly="readonly"  name="totalMoney" id="totalMoney${lengthNum}" class="small text"  value="${decoreOutProduct.price}" checkType="float,1" canEmpty="Y" tip="请输入项目价值，必须为数字" >
									<input type="text" readonly="readonly" name="salerPrice" id="salerPrice${lengthNum}" class="small text" class="small text" value="${decoreOutProduct.salerPrice}"  checkType="float,1" canEmpty="Y" tip="请输入项目价值，必须为数字">
								</td>
								<td style="text-align: center;">
									<input type="text" readonly="readonly"  name="purchasePrice" id="purchasePrice${lengthNum}" class="small text"   checkType="float,1" canEmpty="Y" tip="" >
								</td>
								<td>
									<input type="text" name="note" id="note${lengthNum}" class="mideaX text"  checkType="string,1" canEmpty="Y" value="${decoreOutProduct.note }" tip="请输入项目备注信息" >
								</td>
					...
					<c:forEach var="i" begin="${lengthNum }" step="1" end="7">
						 <tr id="tr${i+1}" onclick="">
								...
								<td style="text-align: center;">
									<input type="text" readonly="readonly" name="price" id="price${i+1}" class="small text"   onkeyup="decorePrice(${i+1})" value="${decoreOutProduct.price}" checkType="float,1" canEmpty="Y" tip="请输入商品单价，必须为数字" >
								</td>
								<td style="text-align: center;">
									<input type="hidden" readonly="readonly"  name="totalMoney" id="totalMoney${i+1}" class="small text"  value="${decoreOutProduct.price}" checkType="float,1" canEmpty="Y" tip="请输入项目价值，必须为数字" >
									<input type="text" readonly="readonly" name="salerPrice" id="salerPrice${i+1}" class="small text" class="small text" value="${decoreOutProduct.salerPrice}"  checkType="float,1" canEmpty="Y" tip="请输入项目价值，必须为数字">
								</td>
								<td style="text-align: center;">
									<input type="text" readonly="readonly"  name="purchasePrice" id="purchasePrice${i+1}" class="small text"   checkType="float,1" canEmpty="Y" tip="" >
								</td>
								<td>
									<input type="text" name="note" id="note${i+1}" class="mideaX text"  checkType="string,1" canEmpty="Y" value="${decoreOutProduct.note }" tip="请输入项目备注信息" >
								</td>
								<td style="text-align: center;padding-top: 8px;">
									<a class="aedit" href="javascript:void(-1)" onclick="deleteTr(this)" >
										<i class="icon-remove icon-black" > </i>
			...
			<tr height="42">
				...
				<td >
					<c:if test="${empty(decoreOut) }">
						<input type="text" name="decoreOut.zkl" id="zkl" onkeyup="disMoneyDis(this.value)" value="100" class="largeX text" title="折扣率" placeholder="100"	checkType="float,0" tip="折扣率不能为空">%<span style="color: red;">*</span>
					</c:if>
					<c:if test="${!empty(decoreOut) }">
						<input type="text" name="decoreOut.zkl" id="zkl" onkeyup="disMoneyDis(this.value)" value="${decoreOut.zkl }" class="largeX text" title="折扣率" placeholder="100"	checkType="float,0" tip="折扣率不能为空">%<span style="color: red;">*</span>
					</c:if>
				</td>
				<td class="formTableTdLeft">折扣后金额:&nbsp;</td>
				<td>
					<c:if test="${empty(decoreOut) }">
						<input type="text" name="decoreOut.disMoney" id="disMoney" value="0" class="largeX text" title="折扣后金额" checkType="float,0" tip="折扣后金额不能为空"><span style="color: red;">*</span>
					</c:if>
					<c:if test="${!empty(decoreOut) }">
						<input type="text" name="decoreOut.disMoney" id="disMoney" value="${decoreOut.disMoney }" class="largeX text" title="折扣后金额" checkType="float,0" tip="折扣后金额不能为空"><span style="color: red;">*</span>
					</c:if>
				</td>
			</tr>
			<tr height="42">
				...
				<td colspan="">
					<input type="text" name="decoreOut.costTotalPrice" id="costTotalPrice" value="${decoreOut.costTotalPrice}"  placeholder="0.0" class="largeX text" title="成本总额"	checkType="float,0" tip="成本总额"><span style="color: red;">*</span>
				</td>
				<td class="formTableTdLeft">顾问结算总额:&nbsp;</td>
				<td >
					<input type="text" name="decoreOut.salerTotalPrice" id="salerTotalPrice" value="${decoreOut.salerTotalPrice }" placeholder="0.0" class="largeX text" title="顾问结算总额"	checkType="float,0" tip="顾问结算总额"><span style="color: red;">*</span>
				</td>
			</tr>
			<tr height="42">
				<td class="formTableTdLeft">实收金额:&nbsp;</td>
				<td colspan="">
					<input type="text" name="decoreOut.acturePrice" id="acturePrice" value="${decoreOut.acturePrice }"  placeholder="0.0" class="largeX text" title="本次付款金额"	checkType="float,0" tip="本次付款金额">
					<span style="color: red;">*</span>
				</td>
				<td class="formTableTdLeft">销售人员:&nbsp;</td>
				<td colspan="">
					<input type="hidden" name="decoreOut.dbid" id="dbid" value="${decoreOut.dbid }" >
	...
	<div class="formButton">
		<a href="javascript:void(-1)"	onclick="$.utile.submitForm('frmId','${ctx}/saleDecoreOut/savePreWashed')"	class="but butSave">保&nbsp;&nbsp;存</a> 
...
function crateTr() {
	...
	var str = ' <tr id="tr'+size+'">'
			...
			+ '<td  style="text-align: center;">'
				...
				+' <input type="text" readonly="readonly" name="salerPrice" id="salerPrice'+size+'" class="small text"checkType="float,1" canEmpty="Y"  >'
			+ '</td>'
			+ '<td style="text-align: center;">'
