...
<div id="hearder_nav" class="views content_title navbar-fixed-top">
    ...
    <a class="go_home" href="${ctx }/qywxFacotoryOrder/index">
								...
								<tr>
									<td id="" width="200">
										<c:if test="${installItem.product.type==1 }">
											${i.index+1 }、<a href='javascript:;' onclick="showDecore('${ctx}/qywxFacotoryOrder/viewProduct?productId=${installItem.product.dbid }','','','商品明细')" >${installItem.itemName }</a>
										</c:if>
										<c:if test="${installItem.product.type==2 }">
											${i.index+1 }、<a href='javascript:;' onclick="showDecore('${ctx}/qywxFacotoryOrder/viewProduct?productId=${installItem.product.dbid }','','','套餐明细')" >${installItem.itemName }</a>
										</c:if>
								    </td>
...
<div class="orderContrac detail">
	...
	<div style="margin: 0 auto;margin: 5px;">
		<div style="color:#8a8a8a;padding-left: 5px; ">
				<c:if test="${empty(specialMotorPool)||specialMotorPool==null }" var="status">
						<span style="color: red;padding-left: 12px;">无占车记录</span>
				</c:if>
				<c:if test="${status==false }">
				<table width="100%" border="0" class="mainTable" cellpadding="0" cellspacing="0">
						<tr height="">
							<td class="formTableTdLeft" >提车人：
