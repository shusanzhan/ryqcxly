...
<c:if test="${status==false }">
	<div class="orderContrac">
		<div class="title" align="left">
			${empty(param.startDate)==true?'今日':''}成交（归档）客户<span style="color: red;font-size: 20px;">${dayCustomerNum}</span>
			<br>
			<p>默认只查询当天成交（归档）客户,您可以通过条件选择查询更多数据</p>
		</div>
	</div>
	<br>
	<c:forEach items="${customers }" var="customer">
		...
		<div class="orderContrac" >
			...
			<div style="margin: 0 auto;margin: 5px;" onclick="window.location.href='${ctx}/qywxCustomer/customerDetail?customerId=${customer.dbid }&type=1'">
				...
				<br>
				vin码:
				<a href="${ctx }/qywxCustomer/factoryOrderDetail?vinCode=${customer.customerPidBookingRecord.vinCode}">${customer.customerPidBookingRecord.vinCode}</a>
			</div>
			</div>
		</div>
	</c:forEach>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-body">
      	<form class="form-inline" action="${ctx }/qywxCustomer/queryLeaderSuccessCustomer" name="frmId" id="frmId" method="post">
      	 <table>
      	 	<tr height="">
      	 	...
      	 	<tr>
      	 		...
      	 		<tr>
      	 		<td width="60"><label for="exampleInputName2">归档日期开始</label></td>
      	 		<td width="240">
      	 			<input type="date" class="form-control" id="startDate" name="startDate" value="${param.startDate }">
			    </td>
      	 	</tr>
      	 	<tr>
      	 		<td width="60"><label for="exampleInputName2">归档日期结束</label></td>
      	 		<td width="240">
