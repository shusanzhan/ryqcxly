...
<title>${customer.name }上传图行驶证</title>
<style type="text/css">
	#img{
		padding: 5px;
	}
...
<div id="hearder_nav" class="views content_title navbar-fixed-top">
    ...
    <span id="page_title">${customer.name }上传行驶证</span>
...
<c:if test="${customerImageApproval.drivingStatus<4 }">
<div class="oneMenu">
	<ul style="width: 90%;">
		<c:if test="${empty(customerImage) }">
			 <li  style="width: 50%;">
        ...
        function upload() {
          wx.uploadImage({
            ...
            success: function (res) {
              ...
              $.post("${ctx}/qywxCustomerImage/saveImageDriving?mediaId="+serverId+"&customerId=${customer.dbid}&num=2",{},function(data){
            	  if (data[0].mark == 0) {// 返回标志为0表示添加数据成功
            	  		alert(data[0].message);
