...
<div class="views content_title navbar-fixed-top">
    ...
    <span id="page_title">待上传照片客户</span>
	...
	<c:forEach items="${customers }" var="customer">
		...
		<div class="orderContrac" >
			<div class="title" align="left">
  			...
  			<div class="line"></div>
			<div style="margin: 0 auto;margin: 5px;" onclick="window.location.href='${ctx}/qywxCustomerImage/uploadImage?customerid=${customer.dbid }&type=1'">
			...
			<div style="margin: 0 auto;margin: 5px;" onclick="window.location.href='${ctx}/qywxCustomerImage/uploadImage?customerid=${customer.dbid }&type=1'">
				<div style="color:#8a8a8a;padding-left: 5px; ">
					上传状态：&#12288;
					<c:if test="${customer.customerImageApproval.status==1 }" var="status">
						<span style="color: red;">等待上传...</span>
					</c:if>
					<c:if test="${customer.customerImageApproval.status==2 }" var="status">
						<span style="color:pink">上传中...</span>
					</c:if>
					<c:if test="${customer.customerImageApproval.status==3 }" var="status">
						<span style="color: red;">上传完成...</span>
					</c:if>
					<br>
				</div>
				<c:if test="${customer.customerImageApproval.status==2 }" var="status">
					...
					<div style="color:#8a8a8a;padding-left: 5px; ">
						...
						<c:if test="${customer.customerImageApproval.drivingStatus==3}">
							<span style="color: red;">审批驳回</span>
							<br>
							行驶证照片驳回原因：${customer.customerImageApproval.drivingApproval}
						</c:if>
      ...
      <div class="modal-body">
      	<form class="form-inline" action="${ctx }/qywxCustomerImage/successCustomer" name="frmId" id="frmId" method="post">
      	 <table>
      	 	...
      	 	<tr>
      	 		<td width="60"><label for="exampleInputName2">VIN码</label></td>
      	 		<td width="240">
      	 			<input type="text" class="form-control" id="vinCode" name="vinCode" value="${param.vinCode }">
			    </td>
      	 	</tr>
      	 	<tr>
...
function ajaxCarSeriy(val){
		if(val==''||val==undefined){
			...
			$("#carModelId option").remove(); 
			$("#carModelId").append("<option value=''>请选择...</option>"); 
			$.post("${ctx}/qywxCustomerImage/ajaxCarSeriyContent?brandId="+val+"&dateTime="+new Date(),{},function (data){
				if(data!="error"){
...
function ajaxCarModel(sel){
	...
	$.post("${ctx}/qywxCustomerImage/ajaxCarModelBySeriyContent?carSeriyId="+value+"&dateTime="+new Date(),{},function (data){
		if(data!="error"){
