...
<br>
<br>
<c:if test="${empty(nextDaycCustomers)||fn:length(nextDaycCustomers)<=0 }" var="status">
	今日无回访客户！
</c:if>
<c:if test="${status==false }">
	<div class="orderContrac">
		<div class="title" align="left">
			今日需回访客户总数：<span style="color: red;font-size: 20px;">${theNextNum }</span>
		</div>
	</div>
	<br>
	<c:forEach items="${nextDaycCustomers }" var="customerTrack" varStatus="i">
		...
		<div class="orderContrac">
			...
			<div style="margin: 0 auto;margin: 5px;" >
				<div style="color:#8a8a8a;padding-left: 5px; ">
					...
					当前意向级别：${customerTrack.beforeCustomerPhase.name}<br>
					登记时间：<fmt:formatDate value="${customer.createFolderTime }"/> <br/>
					到期回访日：<fmt:formatDate value="${customerTrack.nextReservationTime}" pattern="yyyy-MM-dd hh:mm"/><br>
					超时状态:
