...
<div class="views content_title navbar-fixed-top">
    ...
    <span id="page_title">今天需回访客户</span>
	...
	<c:forEach items="${customerTracks }" var="customerTrack" varStatus="i">
		...
		<div class="orderContrac">
			...
			<div style="margin: 0 auto;margin: 5px;height: 30px;line-height: 30px;">
				...
				<a href="${ctx }/qywxCustomerTrack/add?customerId=${customer.dbid}&customerTrackId=${customerTrack.dbid }&typeRedirect=5">跟踪记录</a>
				<c:if test="${customer.lastResult==0 }">
				|
				<a href="${ctx }/qywxCustomerLastBussi/selectResult?customerId=${customer.dbid}&type=2" class="aedit" >成交结果</a>
				</c:if>
			</div>
		</div>
	</c:forEach>
