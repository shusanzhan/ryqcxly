	...
	<c:forEach items="${processFroms }" var="processFrom">
		...
		<div class="orderContrac">
			...
			<div style="margin: 0 auto;margin: 5px;height: 30px;line-height: 30px;">
				...
				<c:if test="${processRun.type==1}">
					<a href="javascript:void(-1)" onclick="window.location.href='${ctx}/qywxProcessRun/dealTask?processFromId=${processFrom.dbid}&customerId=${customer.dbid }'">审批</a>
				</c:if>
				<c:if test="${processRun.type==2}">
					<a href="#" class="aedit" onclick="window.location.href='${ctx }/qywxProcessRun/dealCpidTask?processFromId=${processFrom.dbid}&customerId=${customer.dbid }'">审批</a>
				</c:if>
			</div>
		</div>
	</c:forEach>
