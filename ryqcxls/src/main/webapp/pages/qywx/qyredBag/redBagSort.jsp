...
<div id="hearder_nav" class="views content_title navbar-fixed-top">
    ...
    <span id="page_title">${monthDay }月红包排名</span>
...
<div class="row-fluid">
	<div class="row-fluid" style="text-align: center;margin-bottom: 12px;color:#ed145b;line-height: 40px;width: 200px;margin: 0 auto;">
			...
			<c:forEach var="redBag" items="${redBags }" varStatus="i">
				<c:set value="${total+redBag.totalMoney }" var="total"></c:set>
			</c:forEach>
			<table width="100">
				<tbody>
					<tr>
						<td style="text-align: right;" width="60">
							<img src="${ctx }/images/4.gif" width="40"></img>
						</td>
	...
	<div class="row-fluid" style="text-align: center;border-bottom: 1px solid #ed145b;margin-bottom: 12px;color:#ed145b;">
		<h5 style="text-align: left;font-size: 14px">
			排名明细 
			<c:set value="1" var="status"></c:set>
			<c:set value="1" var="sort"></c:set>
			<c:forEach var="redBag" items="${redBags }" varStatus="i">
				<c:if test="${fn:contains(redBag.recipientName,user.realName)}" >
					<c:set value="${i.index+1 }" var="sort"></c:set>
					<c:set value="2" var="status"></c:set>
				</c:if>
			</c:forEach>
			<c:if test="${status==2 }">
				我的排名：<span style="font-size: 20px;">${sort }</span>
			</c:if>
		</h5>
	</div>
	<c:if test="${empty(redBags) }">
		<div class="alert">无红包记录</div>
	</c:if>
	<c:if test="${!empty(redBags) }">
		<table class="table table-bordered table-striped">
			<tbody>
				<tr>
					<td align="center" width="40">排名</td>
				...
				<c:forEach var="redBag" items="${redBags }" varStatus="i">
					<tr>
						<td id='trTd' align='center'  valign='middle'>
							<c:if test="${i.index==0 }">
								<img src="${ctx }/images/1.gif" width="14"></img>
							</c:if>
							<c:if test="${i.index==1 }">
								<img src="${ctx }/images/2.gif" width="14"></img>
							</c:if>
							<c:if test="${i.index==2 }">
								<img src="${ctx }/images/3.gif" width="14"></img>
							</c:if>
							<c:if test="${i.index>2 }">
								${i.index+1 }
							</c:if>
						...
						<td id='trTd' align='center'  valign='middle'>
							<span style="color: red;">${redBag.totalMoney }</span>
						</td>
					</tr>
				</c:forEach>
			</tbody>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-body">
      	<form class="form-inline" action="${ctx }/qywxRedBag/redBagSort" name="frmId" id="frmId" method="post">
      	 <table>
      	 	<tr>
