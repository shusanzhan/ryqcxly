...
<div id="hearder_nav" class="views content_title navbar-fixed-top">
    ...
    <span id="page_title">展厅流量（年）统计</span>
...
<div id="detail_nav">
     <div class="detail_nav_inner">
         <ul class="clearfix padding10">
           <li class="detail_tap pull_left" id="imgs_tap" onclick="window.location.href='${ctx}/qywxStatCustomerRecord/queryReceptionStat'">月统计</li>
           <li class="detail_tap pull_left select" id="pingjia_tap" onclick="window.location.href='${ctx}/qywxStatCustomerRecord/queryReceptionStatYear'">年统计</li>
      	</ul>
     </div>
 </div>
		...
		<tbody>
			...
			<c:forEach items="${statCustomerRecordTimes }" var="statCustomerRecordTime" varStatus="i">
				<tr>
					...
					<td>
						<fmt:formatDate value="${statCustomerRecordTime.createDate}" pattern="yyyy-MM"/> 
					</td>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-body">
      	<form class="form-inline" action="${ctx }/qywxStatCustomerRecord/queryReceptionStatYear" name="searchPageForm" id="searchPageForm" method="post">
			...
			<c:if test="${fn:length(enterprises)>1 }">
  			...
  			<tr>
				<td><label>登记时间：</label></td>
  				<td>
  					<input class="form-control" id="startTime" name="startTime" onFocus="WdatePicker({isShowClear:true,dateFmt:'yyyy-MM'})" value='${beginDate }' >
  				</td>
  			</tr>
  			<tr>
  				<td><label>~</label></td>
  				<td>
  					<input class="form-control" id="endTime" name="endTime" onFocus="WdatePicker({isShowClear:true,dateFmt:'yyyy-MM'})"  value='${endDate}' >
  				</td>
  			</tr>
  			<tr>
  				<td colspan="2">
