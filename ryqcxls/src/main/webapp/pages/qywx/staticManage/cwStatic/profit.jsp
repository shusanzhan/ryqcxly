...
<div id="hearder_nav" class="views content_title navbar-fixed-top" >
    ...
    <span id="page_title">毛利统计报表</span>
...
<div id="detail_nav">
     <div class="detail_nav_inner">
         <ul class="clearfix padding10">
           <li class="detail_tap3 detail_tap pull_left ${param.type==1?'select':''}" id="imgs_tap" onclick="window.location.href='${ctx}/qywxCwProfitStatical/profit?role=${param.role }&type=1'">日报</li>
           <li class="detail_tap3 detail_tap pull_left ${param.type==2?'select':''}" style="border:1px solid #ff9f29 " id="pingjia_tap" onclick="window.location.href='${ctx}/qywxCwProfitStatical/profit?role=${param.role }&type=2'">月报</li>
           <li class="detail_tap3 detail_tap pull_left ${param.type==3?'select':''}" id="pingjia_tap" onclick="window.location.href='${ctx}/qywxCwProfitStatical/profit?role=${param.role }&type=3'">年报</li>
      	</ul>
     </div>
 </div>
<form class="form-inline" action="${ctx }/qywxCwProfitStatical/profit" name="frmId" id="frmId" method="post">
      	 <table>
      	 	<tr>
      	 		<td><label for="exampleInputName2">查询日期</label>
      	 		...
      	 		<td width="240">
      	 			<c:if test="${param.type==1 }">
	      	 			<c:if test="${!empty(param.day) }">
	      	 				<input type="date" class="middle text" id="day" name="day" value="${param.day }">
	      	 				<%-- <input class="text small" id="day" name="day" onFocus="WdatePicker({isShowClear:true,readOnly:true})" value="${param.day }" > --%>
	      	 		    </c:if>
	      	 		    <c:if test="${empty(param.day) }">
	      	 				<%-- <input class="text small" id="day" name="day" onFocus="WdatePicker({isShowClear:true,readOnly:true})" value="${param.day }" > --%>
	      	 		   		<input type="date" class="middle text" id="day" name="day" value="<%=DateUtil.format(new Date())%>">
	      	 		    </c:if>
      	 			</c:if>
      	 			<c:if test="${param.type==2 }">
	      	 			<c:if test="${!empty(param.month) }">
	      	 				<input type="month" class="middle text" id="month" name="month" value="${param.month }">
	      	 		    </c:if>
	      	 		    <c:if test="${empty(param.month) }">
	      	 				<input type="month" class="middle text" id="month" name="month" value="<%=DateUtil.format(new Date())%>">
	      	 		    </c:if>
      	 			</c:if>
	...
	<table id="shopNumber" border="0" align="center" cellpadding="0" cellspacing="0" style="margin-top: 5px;width: 98%;margin-right: 5px;margin-left: 5px;">
		<tbody>
			<tr height="32" style="background-color: #eeeeee; border-color:#eeeeee;font-size: 14px;font-weight: bold;">
				...
				<td align="center"  width="9%">类型</td>
				<td align="center"  width="9%">合同总金额</td>
				<td align="center"  width="9%">车款金额</td>
				<td align="center"  width="9%">装饰总额</td>
				<td align="center"  width="9%">咨询服务费</td>
				<td align="center"  width="9%">预收总额</td>
				<td align="center" width="9%">车辆成本总额</td>
			</tr>
				...
				<tr>
					<!-- <td align="center" width="9%">序号</td>
					<td align="center"  width="9%">类型</td> -->
					<td align="center" width="9%" style="font-size: 14px;font-weight: bold;">单车利润</td>
					<td align="center"  width="9%" style="font-size: 14px;font-weight: bold;">综合利润</td>
					<td align="center"  width="9%" colspan="5" style="font-size: 14px;font-weight: bold;">综合税后利润</td>
				</tr>
				<c:forEach var="cwCarMoneyStatical" items="${cwCarMoneyStaticals }" varStatus="i">
					<%-- <td align="center">
							${i.index+1 }
