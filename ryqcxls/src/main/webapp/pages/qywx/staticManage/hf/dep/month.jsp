...
<div id="detail_nav">
     <div class="detail_nav_inner">
          <ul class="clearfix padding10">
           <li class="detail_tap2 detail_tap pull_left select" id="pingjia_tap" onclick="window.location.href='${ctx}/qywxHfDep/month?brandId=${brandId}'">月</li>
           <li class="detail_tap2 detail_tap pull_left " id="pingjia_tap" onclick="window.location.href='${ctx}/qywxHfDep/year?brandId=${brandId}'">年</li>
      	</ul>
     </div>
 </div>
...
<div class="row-fluid" style="text-align: center;border-bottom: 1px solid #ed145b;margin-bottom: 12px;color:#ed145b;">
	<h5 style="text-align: left;font-size: 16px">
		${startTime }月自有店部门<span style="font-size: 14px;"></span>
	</h5>
</div>
<div class="row-fluid">
	...
	<c:if test="${!empty(hSalerStats) }">
		<table class="table table-bordered table-striped">
			<tbody>
				...
				<c:forEach var="hSalerStat" items="${ hSalerStats}">
					<tr>
						<td id='trTd' align='center'  valign='middle'>
							<a href='javascript:void(-1)' onclick="artDia('${ctx }/qywxHfDep/salerStaDetail?brandId=${brandId}&startTime=${startTime }&depId=${hSalerStat.depId }','${hSalerStat.depName }部门【${hSalerStat.notstaTotal }】')">${hSalerStat.depName }</a>
						</td>
						<td id='trTd' align='center'  valign='middle'>
							${hSalerStat.total }
						</td>
