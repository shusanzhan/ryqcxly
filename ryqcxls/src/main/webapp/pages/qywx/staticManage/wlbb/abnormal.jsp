...
<div class="views content_title navbar-fixed-top">
    ...
    <span id="page_title">异常车辆统计</span>
...
<ul class="list-group">
<c:if test="${fn:length(roomCarSerCounts)>0 }" var="status">
	 <c:set value="0" var="num"></c:set>
	<c:forEach var="carSerCount" items="${roomCarSerCounts }">
	 <li class="list-group-item" onclick="window.location.href='${ctx}/qywxWlReport/abnormalFacotryOrderList?storeRoomId=${carSerCount.serid }'">
...
<ul class="list-group">
<c:if test="${fn:length(carSeriyCarSerCounts)>0 }" var="status">
	<c:forEach var="carSerCount" items="${carSeriyCarSerCounts }">
	 <li class="list-group-item" onclick="window.location.href='${ctx}/qywxWlReport/abnormalFacotryOrderList?carSeriyId=${carSerCount.serid }'">
	    <span class="badge" id="badgeSelf">
    		<c:if test="${!empty(carSerCount.countNum)}">
	    		${carSerCount.countNum }
	    	</c:if>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-body">
      	<form class="form-inline" action="${ctx }/qywxWlReport/abnormal" name="frmId" id="frmId" method="post">
      	 <table>
      	 	<tr>
