...
<%@page import="com.ystech.xwqr.model.CarSeriy"%>
<%@page import="com.ystech.xwqr.model.carstock.StoreRoom"%>
...
<title>${carSeriy.name }库存车系统计</title>
</head>
<body>
<div class="views content_title navbar-fixed-top">
    ...
    <span id="page_title">${carSeriy.name }库存车系统计</span>
...
<div class="row-fluid" style="text-align: center;border-bottom: 1px solid #ed145b;margin-bottom: 12px;color:#ed145b;">
	<h5 style="text-align: left;font-size: 16px">
		${storeRoom.name } ${carSeriy.name }库存车系统计
	</h5>
</div>
<ul class="list-group">
<c:if test="${fn:length(carSerCounts)>0 }" var="status">
	...
	<c:forEach var="carSerCount" items="${carSerCounts }">
	 <li class="list-group-item" onclick="window.location.href='${ctx}/qywxWlReport/facotryOrderList?carModelId=${carSerCount.serid }&storeRoomId=${storeRoom.dbid }'">
...
<div class="row-fluid" style="text-align: center;border-bottom: 1px solid #ed145b;margin-bottom: 12px;color:#ed145b;">
	<h5 style="text-align: left;font-size: 16px">
		${storeRoom.name } ${carSeriy.name }库龄统计 
	</h5>
</div>
<div class="row-fluid">
...
<%
	...
	CarSeriy carSeriy=(CarSeriy)request.getAttribute("carSeriy");
	...
	for(int i=0;i<carSerCounts.size();i++){
		...
		String sql="SELECT cs.dbid,cs.`name`,COUNT(cs.dbid) AS carSTotal "+
				...
				"WHERE cs.dbid=sf.carSeriyId AND sf.carStatus!=3 AND sf.carSeriyId="+carSeriy.getDbid()+" AND sf.storeRoomId="+storeRoom.getDbid()+" AND sf.storeAgeLevelId="+carSerCount.getSerid()+
