...
<div class="views content_title navbar-fixed-top">
    ...
    <span id="page_title">加装车辆统计</span>
...
<c:if test="${fn:length(roomCarSerCounts)>0 }" var="status">
	...
	<c:forEach var="carSerCount" items="${roomCarSerCounts }">
	 <li class="list-group-item" onclick="window.location.href='${ctx}/qywxWlReport/installFacotryOrderList?storeRoomId=${carSerCount.serid }'">
...
<c:if test="${fn:length(carSeriyCarSerCounts)>0 }" var="status">
	<c:forEach var="carSerCount" items="${carSeriyCarSerCounts }">
	 <li class="list-group-item" onclick="window.location.href='${ctx}/qywxWlReport/installFacotryOrderList?carSeriyId=${carSerCount.serid }'">
	    <span class="badge" id="badgeSelf">
    		<c:if test="${!empty(carSerCount.countNum)}">
	    		${carSerCount.countNum }
	    	</c:if>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-body">
      	<form class="form-inline" action="${ctx }/qywxWlReport/install" name="frmId" id="frmId" method="post">
      	 <table>
      	 	<tr>
