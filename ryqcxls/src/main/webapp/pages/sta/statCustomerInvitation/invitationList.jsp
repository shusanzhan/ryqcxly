...
<div class="listOperate">
	<div class="operate">
		<a href="javascript:void(-1)"  onclick="window.location.href='${ctx}/statCustomerInvitation/queryInvitationYearList'" style="color: #2b7dbc;">网销线索年统计</a> <span style="color:#3eb94e ">|</span>
		...
		<a href="javascript:void(-1)" onclick="window.history.go(-1)">返回</a>
   </div>
  	<div class="seracrhOperate" style="margin: 20px 1px;">
  		<form name="searchPageForm" id="searchPageForm"  action="${ctx}/statCustomerInvitation/queryInvitationList" method="post" >
...
$(function () {
	...
	$('#pieCustomerInfromStaSet').highcharts({
		...
		title: {
			text: '${beginDate }至${endDate }当月潜客各网销平台占比'
		},
		tooltip: {
	...
	$('#pieCustomerInfromComeShopStaSet').highcharts({
		...
		title: {
			text: '${beginDate }至${endDate }当月潜客各网销平台邀约占比'
		},
		tooltip: {
			pointFormat: '{series.name}: <b>{point.percentage:.1f}%</b>'
		},
	...
	$('#pieUserMonth').highcharts({
		...
		title: {
			text: '${beginDate }至${endDate }当月潜客各销售顾问占比'
		},
		tooltip: {
			pointFormat: '{series.name}: <b>{point.percentage:.1f}%</b>'
		},
	...
	$('#pieUserMonthComeShop').highcharts({
		...
		title: {
			text: '${beginDate }至${endDate }当月潜客各销售顾问邀约占比'
		},
		tooltip: {
			pointFormat: '{series.name}: <b>{point.percentage:.1f}%</b>'
		},
...
function ajaxCarCount(){
	...
	$.post("${ctx}/statCustomerInvitation/ajaxCarCount?customerInfromSetDbid="+customerInfromSetDbid+"&dateType=1",params,function(data){
		if(data=="error"){
			alert("查询出错！");
		}else{
			var obj=data.split("|");
