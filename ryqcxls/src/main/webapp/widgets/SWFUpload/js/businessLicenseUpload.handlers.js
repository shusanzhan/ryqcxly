	...
	try {
		...
		if(null!=serverData&&undefined!=serverData&&serverData.length>0){
			...
			if(returnResult[0]=="success"){//上传文件成功
				var img="<img src='"+returnResult[1].replace("source","500_360")+"'></img>"
				$("#contentImage").text("");
				$("#contentImage").html(img);
				var length=returnResult[1].lastIndexOf("/");
				$("#credentialsShow").val(returnResult[1].substring(length+1,returnResult[1].length));
				$("#credentials").val(returnResult[1]);
			}else{
				$("#message").text("");
