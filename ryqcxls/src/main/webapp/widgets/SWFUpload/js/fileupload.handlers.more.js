...
function fileQueueErrorHandler(file, errorCode, message) {
	...
	try {
		if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
			alert("当前只能上传.: "+swfUpload.settings.file_upload_limit+" 张图片!");
			return;
		}
		switch (errorCode) {
...
function uploadStart(file) {
	try {
		var uploadState="<span id='state' class='stateWating'>开始上传...</span>";
	...
	try {
		...
		if(null!=serverData&&undefined!=serverData&&serverData.length>0){
			...
			if(returnResult[0]=="success"){//上传文件成功
				var resultValue='<input type="hidden" id="attach'+file.id+'" name="attach" value="'+returnResult[1]+'">';
				var uploadState="<span id='fileSize'>文件大小："+fileSize+"</span><span id='state' class='stateSuccess'>上传成功</span>";
				$("#uploadState"+file.id).text("");
				$("#uploadState"+file.id).append(uploadState);
				$("#uploadState"+file.id).append(resultValue);
...
//删除文件上传状态：uploadState有三种状态一、等待上传waiting；二、正在上传状态uploading；三、上传完成状态uploaded
function deleteFile(file,uploadState,fileUrl){
	try{
		if(undefined!=uploadState&&uploadState.length>0){
			switch (uploadState) {
				case "waiting":
					swfUpload.cancelUpload(file.id);  
					totalNum=totalNum-1;
					...
					break;
				case "error":
					var totalNum=parseInt($("#totalNum").text(),10) ;  
					alert($("#totalNum").text());
					totalNum=totalNum-1;
					if(totalNum<=0){//没有文件时，清空总的统计数据
						$("#amount").text("");
				...
				case "uploaded":
					//调用后台删除文件程序
					$.post('${ctx}/swfUpload/deleteFile?fileUrl='+encodeURI(encodeURI(fileUrl))+"&timeStamp="+new Date(),function(data){
						if(data=="success"){
							totalNum=totalNum-1;
							if(totalNum<=0){//没有文件时，清空总的统计数据
								$("#amount").text("");
