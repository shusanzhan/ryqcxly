	...
	try {
		...
		if(null!=serverData&&undefined!=serverData&&serverData.length>0){
			...
			if(returnResult[0]=="success"){//上传文件成功
				...
				$("#result").val(result);
				var uploadState="<span><img src='/images/imageUpload/icons3.png' /><a href='#' id='deleteMethod"+file.id+"'>删除</a></span></div>";
				$("#fileContent"+file.id).css("background-image","url("+encodeURI(returnResult[1])+")");
				$("#fileContent"+file.id).text("");
				$("#fileContent"+file.id).append(uploadState);
				//从新绑定删除方法
