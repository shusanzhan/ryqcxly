...
if (typeof(SWFUpload) === "function") {
	...
	SWFUpload.speed.getBytes = function (baseNumber){
		var sizeUnits = [1073741824, 1048576, 1024, 1], sizeUnitLabels = ["GB", "MB", "KB", "bytes"];
		if(baseNumber.indexOf("GB")>0){
			var length=baseNumber.indexOf("GB");
			return sizeUnits[0]*parseFloat(baseNumber.substring(0,length));
		}
		if(baseNumber.indexOf("MB")>0){
			var length=baseNumber.indexOf("MB");
			return sizeUnits[1]*parseFloat(baseNumber.substring(0,length));
		}
		if(baseNumber.indexOf("KB")>0){
			var length=baseNumber.indexOf("KB");
			return sizeUnits[2]*parseFloat(baseNumber.substring(0,length));
		}
		if(baseNumber.indexOf("bytes")>0){
			var length=baseNumber.indexOf("bytes");
			return sizeUnits[3]*parseFloat(baseNumber.substring(0,length));
		}
	}
