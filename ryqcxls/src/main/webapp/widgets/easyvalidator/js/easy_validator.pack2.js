...
/*	var normal=obj.attr("class");
	...
	var normal=$(obj).parent().parents('.control-group').attr("class");
	var fail=" error";
	var success=" success";
	if (action_type == "add") {//添加错误样式
		if(normal.indexOf("error")<0){//如果包含错误样式
			/*$(obj).parent().parents('.control-group').removeClass(fail);
			$(obj).parent().parents('.control-group').addClass(success);*/
			$(obj).parent().parents('.control-group').removeClass(success);
			$(obj).parent().parents('.control-group').addClass(fail);
		}
	} else {//删除错我样式
		$(obj).parent().parents('.control-group').removeClass(fail);
		$(obj).parent().parents('.control-group').addClass(success);
