...
/*	if(url!=undefined){
	...
	else if (/^dateComp/.test(obj.attr('checkType'))) {
		if (!dateComp(obj)) {
			change_error_style(obj, "add");
			change_tip(obj, null, "remove");
			return false;
...
function change_error_style(obj, action_type) {
	var warm="<div class='weui_cell_ft'>"+
              "<i class='weui_icon_warn'></i>"+
            "</div>";
	var normal=$(obj).parent().parent('.weui_cell').attr("class");
	var fail=" weui_cell_warn";
	var success=" success";
	if (action_type == "add") {//添加错误样式
		if(normal.indexOf("weui_cell_warn")<0){//如果包含错误样式
			$(obj).parent().parents('.weui_cell').removeClass(fail);
			$(obj).parent().parents('.weui_cell').removeClass(success);
			$(obj).parent().parents('.weui_cell').addClass(fail);
			$($(obj).parent().parents('.weui_cell')).append(warm);
		}
	} else {//删除错我样式
		$(obj).parent().parents('.weui_cell').removeClass(fail);
		$(obj).parent().parents('.weui_cell').addClass(success);
		$($(obj).parent().parents('.weui_cell')).find(".weui_cell_ft").remove();
	}
}
...
function dateComp(obj) {
	...
	if(date==null){
		...
		if(null==value2){
			...
			if(value2.indexOf(":")!=-1){
				day2=stringToYYYYMMDDHHMM(new Date().format("yyyy-MM-dd")+" "+value2);
			}else{
				day2=new Date(value2);
			}
		}
	}
	...
	if(value.indexOf(":")!=-1){
		 ...
		 var vale=fm+" "+value;
		 day1=stringToYYYYMMDDHHMM(vale);;
	}else{
		day1=new Date(value);
	}
	valid=dateCompare(day1,day2,type);
	return valid;
}
/**
 * 输入框只能输入中文
