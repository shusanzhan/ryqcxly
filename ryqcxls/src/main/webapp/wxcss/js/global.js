...
var YS = {
    ...
    data: {},
    domain: {
        web: typeof web_domain == 'undefined' ? '' : web_domain,
        stc: typeof stc_domain == 'undefined' ? '' : stc_domain,
        res: typeof res_domain == 'undefined' ? '' : res_domain
    },
};
...
YS.ui.tips = {
    ...
    suc: function (msg, url) {
        this._set(msg, url, { useOverlay: true, position: null, autoclose: 1000, icon: "check", closeOverlay: true, buttons: null })
    },
    info: function (msg, url) {
        this._set(msg, url, {
            useOverlay: true, position: "center", icon: "info", autoclose: null, closeOverlay: true, buttons: url ? {
                button1: {
    ...
    confirm_flag: function (msg, func) {
        $.fallr('show', {
            buttons: {
                button1: {
                    text: '确定', danger: true, onclick: func
                },
                button2: {
                    text: '取消'
                }
    ...
    /* 设置cookie */
    up: function (msg, value, url) {
        // var item = $.cookie("up_tips");
        // if (!item || item != value) {
            var btn = {
                button1: {
                    text: '关闭', onclick: function () {
                        // $.cookie("up_tips", value, { expires: 365, path: "/" });
                        $.fallr('hide');
                    }
                },
                button2: {
...
YS.logic.form = {
    ...
    tip: function (data) {
        if (data.errno == 0) {
            if (data.tip) {
                YS.ui.tips.html(data.tip, data.url);
            }
            else {
                YS.ui.tips.suc(data.error, data.url)
            }
        } else {
            YS.ui.tips.err(data.error, data.url)
        }
    },
                          ...
                          if (typeof KindEditor !== "undefined" && KindEditor.instances) {
                              $.each(KindEditor.instances, function () {
                                  ...
                                  if (r) {
                                      if (v.length == 0) {
                                          var msg = m;
                                          YS.ui.tips.info(msg ? msg : "内容不能为空")
                                  ...
                                  if (e) {
                                      ...
                                      if (vv.length <= min || vv.length >= max) {
                                          YS.ui.tips.info(em ? em : "内容不能小于{0}且大于{1}".format(min, max));
                                          _sb = false; return false;
                                      }
                          ...
                          if (_sb) {
                              ...
                              $(form).ajaxSubmit({
                                  ...
                                  error: function (d) {
                                      btn.button('reset');
                                      if (modal) modal.modal('hide');
                                      YS.ui.tips.info(d.responseText);
                                  }
                              });
            ...
            btn.click(function () {
                ...
                $from.ajaxSubmit({
                    ...
                    success: function (data) {
                        ...
                        YS.logic.form.tip(data);  /* 要点2 */
                    },
                    error: function (data) {
                        btn.button('reset');
                        btn.parents(".modal").modal('hide');
                        YS.ui.tips.info(data.responseText);  /* 要点3 */
                    }
                });
...
YS.logic.page = {
    ...
    common: function () {
        ...
        if ($talbecheckbox.length > 0) {
            $("table.ajax-checkbox input[type='checkbox'][name='show']").click(function () {
                ...
                if (f) {
                    ...
                    if (l && l != 0) {
                        var tmp = "最多勾选{0}个";
                        if (ck_num > l) { YS.ui.tips.info(tmp.format(l)); return false; }
                    }
        ...
        $("div.location_box").length > 0 && $("div.location_box").empty().append(YS.util.location.locSelection($("div.location_box").attr("data-district")));
    ...
    copy: function () {
        ZeroClipboard.setDefaults({
            moviePath: YS.domain.res + "/swf/zeroclipboard.swf"
        });
        $(".copy_text").each(function (i) { 
        	var copy_text = $(this).data('text') ? $(this).data('text') : '复制';
            var $id = "copy_button{0}".format(i); 
            var _tmp = '  <button class="btn copy" id="{0}" type="button" data-clipboard-target="{2}" data-clipboard-text="{1}"><i class="icon-share"></i> ' + copy_text + '</button>  <span class="alert copy-success help-inline alert-success hide "  >复制成功,请粘帖到您需要的地方</span>';
...
YS.util.location = {
    ...
    getLocInfo: function (district) {
        if (!YS.util.area) return false;

        var self = YS.util.location;
        if (self._loc === false) {
            self._loc = {};
            $.each(YS.util.area, function (pid, pinfo) {
    ...
    locSelection: function (district) {
        ...
        p.change((function (_, __, ___) {
            return function () {
                ...
                if (YS.util.area[pid]) {
                    $.each(YS.util.area[pid].l, function (cid, cinfo) {
        ...
        c.change((function (_, __, ___) {
            return function () {
                ...
                if (YS.util.area[pid] && YS.util.area[pid].l[cid]) {
                    var r = $.extend({}, YS.util.area[pid].l[cid].l);
                    var l = [];
                    $.each(r, function (k, v) {
                        l.push({
                            id: k,
        ...
        j.setLocation = function (dist) {
            ...
            var ddInfo = YS.util.location.getLocInfo(dist);
            if (ddInfo !== false) {
...
/* select工具 */
YS.util.select = function (options) {
...
// if (!((/weimob.com/i.test(window.location)))) YS.domain.t = "/static", YS.domain.k = "/wm-xin-a";
